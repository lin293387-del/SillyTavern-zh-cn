...
const extensionTaskQueue = createTaskQueue('extensions:tasks');
app.set('queues:extensions', extensionTaskQueue);
process.once('exit', () => extensionTaskQueue.close());

extensionTaskQueue.setProcessor(async (task) => {
    const listeners = serverEvents.listeners?.(EVENT_NAMES.EXTENSION_TASK) ?? [];
    if (!listeners.length) {
        console.warn('Extension task enqueued but no handlers are registered. Dropping task.', task);
        return;
    }
    ...
    await Promise.allSettled(listeners.map(async (listener) => {
        try {
            ...
            console.error('Extension task handler failed', error);
        }
    }));
});

const importTaskQueue = createTaskQueue('import:parse');
app.set('queues:import', importTaskQueue);
process.once('exit', () => importTaskQueue.close());
registerImportProcessors(importTaskQueue);

const defaultJsonLimitMb = Math.max(1, Number(getConfigValue('server.defaultJsonLimitMb', 25, 'number')) || 25);
const defaultUrlencodedLimitMb = Math.max(1, Number(getConfigValue('server.defaultUrlencodedLimitMb', 25, 'number')) || 25);

const toMbString = (valueMb) => `${Math.max(1, Number(valueMb) || 1)}mb`;
const createJsonParser = (limitMb = defaultJsonLimitMb) => bodyParser.json({ limit: toMbString(limitMb) });
const createUrlencodedParser = (limitMb = defaultUrlencodedLimitMb) => bodyParser.urlencoded({ extended: true, limit: toMbString(limitMb) });
...
if (!cliArgs.disableCsrf) {
    const csrfSyncProtection = csrfSync({
        ...
        getTokenFromRequest: (req) => {
            ...
            if (!header) {
                console.warn(`(CSRF warn) Missing X-CSRF-Token header for ${req.method} ${req.originalUrl}`);
            }
    ...
    app.use((err, req, res, next) => {
        if (err && err === csrfSyncProtection.invalidCsrfTokenError) {
            const provided = req.headers['x-csrf-token'] || '<missing>';
            let sessionToken = undefined;
            try {
                sessionToken = req.session?.csrfToken || '<missing>';
            } catch {
                sessionToken = '<unavailable>';
            }

            console.error(
                `(CSRF error) Invalid token for ${req.method} ${req.originalUrl} | header=${provided} | session=${sessionToken} | cookies=${JSON.stringify(req.cookies ?? {})}`
            );
        }
        next(err);
    });
...
// Body parsers should run after cookie/session & CSRF so the token state survives.
app.use(createJsonParser());
app.use(createUrlencodedParser());

const exposeMetrics = getConfigValue('server.exposeMetrics', false, 'boolean');
if (exposeMetrics) {
    console.info(color.green('Metrics endpoint enabled at /metrics/json'));
    app.use('/metrics', metricsRouter);
} else {
    app.use('/metrics', (_req, res) => res.status(404).json({ error: 'Metrics endpoint disabled. Enable via server.exposeMetrics.' }));
}
...
app.get('/api/meta/api-toggles', (_req, res) => {
    res.json({
        toggles: getAllApiToggles(),
...
const chatJsonLimitMb = Math.max(1, Number(getConfigValue('server.chatJsonLimitMb', 200, 'number')) || 200);
const chatCompletionsJsonLimitMb = Math.max(1, Number(getConfigValue('server.chatCompletionJsonLimitMb', 50, 'number')) || 50);
const dataMaidJsonLimitMb = Math.max(1, Number(getConfigValue('server.dataMaidJsonLimitMb', 150, 'number')) || 150);
const extensionsJsonLimitMb = Math.max(1, Number(getConfigValue('server.extensionsJsonLimitMb', 25, 'number')) || 25);

app.use('/api/chats', createJsonParser(chatJsonLimitMb));
app.use('/api/backends/chat-completions', createJsonParser(chatCompletionsJsonLimitMb));
app.use('/api/data-maid', createJsonParser(dataMaidJsonLimitMb));
app.use('/api/extensions', createJsonParser(extensionsJsonLimitMb));
...
const maxFieldSizeMb = Math.max(1, Number(getConfigValue('uploads.maxFieldSizeMb', 200, 'number')) || 200);
const avatarFileSizeMb = Math.max(1, Number(getConfigValue('uploads.avatarMaxFileSizeMb', 10, 'number')) || 10);
app.use(multer({
    dest: uploadsPath,
    limits: {
        fieldSize: maxFieldSizeMb * 1024 * 1024,
        fileSize: avatarFileSizeMb * 1024 * 1024,
    },
}).single('avatar'));
console.info(color.green(`Upload limits → fieldSize: ${maxFieldSizeMb}MB, avatar fileSize: ${avatarFileSizeMb}MB`));
