...
import { getApiToggle } from './config-json.js';
...
export function setupPrivateEndpoints(app) {
    ...
    if (getApiToggle('novelAI')) {
        app.use('/api/novelai', novelAiRouter);
    } else {
        console.warn('[API toggle] NovelAI 接口已禁用');
    }
    ...
    if (getApiToggle('aiHorde')) {
        app.use('/api/horde', hordeRouter);
    } else {
        console.warn('[API toggle] AI Horde 接口已禁用');
    }
    ...
    if (getApiToggle('textCompletion')) {
        app.use('/api/backends/text-completions', textCompletionsRouter);
    } else {
        console.warn('[API toggle] 文本补全接口已禁用');
    }
    app.use('/api/openrouter', openRouterRouter);
    if (getApiToggle('kobold')) {
        app.use('/api/backends/kobold', koboldRouter);
    } else {
        console.warn('[API toggle] Kobold 接口已禁用');
    }
    if (getApiToggle('chatCompletion')) {
        app.use('/api/backends/chat-completions', chatCompletionsRouter);
    } else {
        console.warn('[API toggle] 聊天补全接口已禁用');
    }
