...
const directoryCacheLimit = Math.max(1, Number(getConfigValue('users.directoryCacheLimit', 5000, 'number')) || 5000);
const directoryCacheTtlMinutes = Math.max(0, Number(getConfigValue('users.directoryCacheMinutes', 30, 'number')) || 0);
const directoryCacheTtlMs = directoryCacheTtlMinutes > 0 ? directoryCacheTtlMinutes * 60 * 1000 : 0;

/**
 * Cache for user directories.
 */
const DIRECTORIES_CACHE = createCache('users:directories', {
    ttlMs: directoryCacheTtlMs,
    maxEntries: directoryCacheLimit,
});
