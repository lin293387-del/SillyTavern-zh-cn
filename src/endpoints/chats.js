 ...
 */
const backupFunctionCacheLimit = Math.max(1, Number(getConfigValue('backups.chat.handleCacheLimit', 1000, 'number')) || 1000);
const backupFunctionCacheTtlMs = Math.max(throttleInterval * 6, 5 * 60 * 1000);
const backupFunctions = createCache('chats:backup-functions', {
    ttlMs: backupFunctionCacheTtlMs,
    maxEntries: backupFunctionCacheLimit,
    onEvict: (throttledFn) => throttledFn?.flush?.(),
});
 ...
 */
async function streamCopyFile(source, destination) {
    await fs.promises.mkdir(path.dirname(destination), { recursive: true });
    await streamPipeline(fs.createReadStream(source), fs.createWriteStream(destination));
}
...
router.post('/group/import', async function (request, response) {
    ...
    try {
        ...
        pathToUpload = path.join(filedata.destination, filedata.filename);
        const pathToNewFile = path.join(request.user.directories.groupChats, `${chatname}.jsonl`);
        await streamCopyFile(pathToUpload, pathToNewFile);
...
router.post('/import', validateAvatarUrlMiddleware, async function (request, response) {
    ...
    let pathToUpload;
    try {
        pathToUpload = path.join(request.file.destination, request.file.filename);
        ...
        if (format === 'jsonl') {
            ...
            if (flattenedChat !== data) {
                ...
                await streamCopyFile(pathToUpload, filePath);
            }
            return response.send({ res: true });
        }
    } catch (error) {
...
export { importOobaChat, importCAIChat, importKoboldLiteChat, importAgnaiChat, importRisuChat, flattenChubChat };
