...
router.post('/delete', validateAvatarUrlMiddleware, async function (request, response) {
    ...
    if (!fs.existsSync(avatarPath)) {
        console.warn(`[characters/delete] Avatar ${request.body.avatar_url} not found. Treating as already deleted.`);
    } else {
        try {
            fs.unlinkSync(avatarPath);
        } catch (error) {
            console.error('Failed to remove character file:', error);
            return response.status(500).send({ message: 'Failed to delete character file', error: error.toString() });
        }
    }
    invalidateThumbnail(request.user.directories, 'avatar', request.body.avatar_url);
    ...
    if (request.body.delete_chats == true) {
        try {
            ...
            console.error('Failed to remove chats folder:', err);
            // continue – chats may have been already removed or unreadable; do not block character deletion
        }
    }
...
export { convertToV2, readFromV2, unsetPrivateFields };
