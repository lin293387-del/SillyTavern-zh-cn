...
const tokenizerCacheLimit = Math.max(0, Number(getConfigValue('tokenizers.cacheLimit', 8, 'number')) || 0);
const tokenizerCacheTtlMinutes = Math.max(0, Number(getConfigValue('tokenizers.cacheRetentionMinutes', 60, 'number')) || 0);
const tokenizerCacheTtlMs = tokenizerCacheTtlMinutes > 0 ? tokenizerCacheTtlMinutes * 60 * 1000 : 0;

const tokenizersCache = createCache('tokenizers', {
    ttlMs: tokenizerCacheTtlMs,
    maxEntries: tokenizerCacheLimit,
    onEvict: (entry, model) => disposeTokenizer(entry, model),
});

function flushTokenizerEvictions() {
    const stats = tokenizersCache.consumeEvictionStats();
    if (stats.ttl > 0) {
        console.debug('Evicted tokenizer instances due to TTL:', stats.ttl);
    }
    if (stats.lru > 0) {
        console.debug('Evicted tokenizer instances due to cache limit:', stats.lru);
    }
}

function disposeTokenizer(entry, model) {
    try {
        entry?.instance?.free?.();
    } catch (error) {
        console.warn('Failed to dispose tokenizer', model, error);
    }
}
