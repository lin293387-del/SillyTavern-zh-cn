 ...
 */
const autosaveCacheLimit = Math.max(1, Number(getConfigValue('backups.settings.handleCacheLimit', 1000, 'number')) || 1000);
const autosaveCacheTtlMs = AUTOSAVE_INTERVAL * 4;
const AUTOSAVE_FUNCTIONS = createCache('settings:autosave-functions', {
    ttlMs: autosaveCacheTtlMs,
    maxEntries: autosaveCacheLimit,
    onEvict: fn => fn?.flush?.(),
});
