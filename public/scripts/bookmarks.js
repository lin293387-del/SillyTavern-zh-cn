...
export async function createBranch(mesId) {
    if (!chat.length) {
        toastr.warning('聊天为空。', '创建分支失败');
        return;
    }

    if (mesId < 0 || mesId >= chat.length) {
        toastr.warning('消息 ID 无效。', '创建分支失败');
        return;
    }
...
export async function createNewBookmark(mesId, { forceName = null } = {}) {
    if (this_chid === undefined && !selected_group) {
        toastr.info('尚未选择角色。', '创建检查点');
        return null;
    }
    if (!chat.length) {
        toastr.warning('聊天为空。', '创建检查点');
        return null;
    }
    if (!chat[mesId]) {
        toastr.warning('消息 ID 无效。', '创建检查点');
        return null;
    }
    ...
    toastr.success('点击消息旁的旗帜图标即可打开检查点聊天。', '创建检查点', { timeOut: 10000 });
    return name;
}
...
function registerBookmarksSlashCommands() {
    ...
    function validateMessageId(mesId, context) {
        if (isNaN(mesId)) {
            toastr.warning('提供的消息 ID 无效', context);
            return false;
        }
        if (!chat[mesId]) {
            toastr.warning(`未找到 ID 为 ${mesId} 的消息`, context);
            return false;
        }
        return true;
    }
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        callback: async (args, text) => {
            ...
            if (typeof text !== 'string') {
                toastr.warning('检查点名称需为字符串或留空', '创建检查点');
                return '';
            }
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        callback: async (args, text) => {
            ...
            if (!checkPointName) {
                toastr.warning('选中的消息未关联任何检查点', '打开检查点');
                return '';
            }
...
export function initBookmarks() {
    ...
    const handleBookmarkFollow = async function (e) {
        const $target = $(this);
        ...
        const fileName = $target.hasClass('mes_bookmark')
            ? $target.closest('.mes').attr('bookmark_link')
            : $target.attr('file_name')?.replace('.jsonl', '');
    ...
    $(document).on('click', '.select_chat_block', handleBookmarkFollow);
    chatContainer.on('click', '.mes_bookmark', handleBookmarkFollow);

    chatContainer.on('click', '.mes_create_bookmark', async function () {
    ...
    chatContainer.on('click', '.mes_create_branch', async function () {
        const mesId = $(this).closest('.mes').attr('mesid');
        if (mesId !== undefined) {
