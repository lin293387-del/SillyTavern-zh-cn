...
function filterGroupMembers(value) {
    const searchValue = String(value ?? '').toLowerCase();
    groupCandidatesFilter.setFilterData(FILTER_TYPES.SEARCH, searchValue);
}

const filterGroupMembersDebounced = debounce(filterGroupMembers, debounce_timeout.quick);
...
jQuery(() => {
    const rightNavPanel = $('#right-nav-panel');
    const groupChatsBlock = $('#rm_group_chats_block');

    if (!CSS.supports('field-sizing', 'content')) {
        groupChatsBlock.on('input', '.autoSetHeight', function () {
            resetScrollHeight($(this));
        });
    }

    rightNavPanel.on('click', '.group_select', function () {
        const groupId = $(this).attr('data-chid') || $(this).attr('data-grid');
        openGroupById(groupId);
    });
    $('#rm_group_filter').on('input', function () {
        filterGroupMembersDebounced($(this).val());
    });
    ...
    rightNavPanel.on('click', '.group_member .right_menu_button', onGroupActionClick);
});
