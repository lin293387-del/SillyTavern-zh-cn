...
const DOCUMENT_WORKER_TASK = 'document-parse';

registerWorkerTask(DOCUMENT_WORKER_TASK, {
    script: 'document-worker.js',
    fallback: documentWorkerFallback,
});
 ...
 * @param {number} [wait=16] Time window in milliseconds.
 * @param {{leading?: boolean, trailing?: boolean}} [options={}] Configure leading/trailing invocation.
 * @returns {Function} The throttled function with cancel/flush helpers.
 */
export function throttle(func, wait = 16, { leading = true, trailing = true } = {}) {
...
async function dispatchDocumentTask(payload) {
    try {
        const result = await workerManager.dispatchTask(DOCUMENT_WORKER_TASK, payload, {
        ...
        const fallbackResult = await documentWorkerFallback(payload, error);
        if (fallbackResult && typeof fallbackResult.text === 'string') {
...
export async function extractTextFromPDF(blob) {
    ...
    try {
        return await dispatchDocumentTask({
            kind: 'pdf',
            blob,
            mimeType: blob.type || 'application/pdf',
        });
    } catch (error) {
        console.error('[Document] PDF 文本提取失败，已回退为空文本', error);
        return '';
    }
}
...
export async function extractTextFromMarkdown(blob) {
    ...
    try {
        ...
        console.error('[Document] Markdown 文本提取失败，已回退为空文本', error);
        return '';
    }
}
...
export async function extractTextFromEpub(blob) {
    ...
    try {
        ...
        console.error('[Document] EPUB 文本提取失败，已回退为空文本', error);
        return '';
    }
}

async function extractTextFromPDFLocal(blob) {
...
async function extractTextFromMarkdownLocal(blob) {
    const markdown = await blob.text();
    return postProcessText(markdown, false);
}

async function extractTextFromEpubLocal(blob) {
    if (!('ePub' in window)) {
...
async function documentWorkerFallback(payload) {
    const kind = payload?.kind;
    let blob = payload?.blob;

    if (!(blob instanceof Blob) && payload?.buffer) {
        blob = new Blob([payload.buffer], { type: payload?.mimeType || 'application/octet-stream' });
    }
    ...
    switch (kind) {
        case 'pdf':
            return { text: await extractTextFromPDFLocal(blob) };
        case 'markdown':
            return { text: await extractTextFromMarkdownLocal(blob) };
        case 'epub':
            return { text: await extractTextFromEpubLocal(blob) };
        default:
            return { text: '' };
    }
}
