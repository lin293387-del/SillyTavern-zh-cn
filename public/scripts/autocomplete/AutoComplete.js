...
const getStScriptSettings = () => globalThis.power_user?.stscript ?? {};
const getAutocompleteSettings = () => getStScriptSettings().autocomplete ?? {};
const getAutocompleteWidthConfig = () => {
    const defaults = {
        left: AUTOCOMPLETE_WIDTH.CHAT,
        right: AUTOCOMPLETE_WIDTH.CHAT,
    };
    const width = getAutocompleteSettings().width ?? {};
    return {
        left: Number.isInteger(width.left) ? width.left : defaults.left,
        right: Number.isInteger(width.right) ? width.right : defaults.right,
    };
};
const getSelectMask = () => {
    const mask = getAutocompleteSettings().select;
    const defaultMask = AUTOCOMPLETE_SELECT_KEY.ENTER | AUTOCOMPLETE_SELECT_KEY.TAB;
    return Number.isInteger(mask) ? mask : defaultMask;
};
...
export class AutoComplete {
    ...
    get matchType() {
        return getStScriptSettings().matching ?? 'fuzzy';
    }

    get autoHide() {
        return getAutocompleteSettings().autoHide ?? false;
    }
    ...
    updatePosition() {
        if (this.isFloating) {
            ...
            const widthConfig = getAutocompleteWidthConfig();
            const leftRect = rect[widthConfig.left] ?? rect[AUTOCOMPLETE_WIDTH.CHAT];
            const rightRect = rect[widthConfig.right] ?? rect[AUTOCOMPLETE_WIDTH.CHAT];
            ...
            if (this.isShowingDetails) {
                ...
                this.domWrap.style.setProperty('--leftOffset', `max(1vw, ${leftRect?.left ?? 0}px)`);
                this.domWrap.style.setProperty('--rightOffset', `calc(100vw - min(${rightRect?.right ?? window.innerWidth}px, ${this.isShowingDetails ? 74 : 0}vw))`);
            } else {
                this.domWrap.style.setProperty('--leftOffset', `max(1vw, ${leftRect?.left ?? 0}px)`);
                this.domWrap.style.setProperty('--rightOffset', `calc(100vw - min(99vw, ${rightRect?.right ?? window.innerWidth}px))`);
            }
        }
        ...
        if (this.isShowingDetails || !this.isReplaceable) {
            if (this.isFloating) {
                ...
                if (this.isReplaceable) {
                    ...
                    this.detailsWrap.style.setProperty('--bottomOffset', `calc(100vh - ${rect[AUTOCOMPLETE_WIDTH.INPUT].top}px)`);
                    this.detailsWrap.style.setProperty('--leftOffset', `${leftRect?.left ?? 0}px`);
                    this.detailsWrap.style.setProperty('--rightOffset', `calc(100vw - ${rightRect?.right ?? window.innerWidth}px)`);
                }
            }
        }
    }
    ...
    async handleKeyDown(evt) {
        const selectMask = getSelectMask();
        ...
        if (this.isActive && this.isReplaceable) {
            ...
            switch (evt.key) {
                ...
                case 'Enter': {
                    ...
                    if ((selectMask & AUTOCOMPLETE_SELECT_KEY.ENTER) !== AUTOCOMPLETE_SELECT_KEY.ENTER) break;
                ...
                case 'Tab': {
                    // pick the selected item to autocomplete
                    if ((selectMask & AUTOCOMPLETE_SELECT_KEY.TAB) !== AUTOCOMPLETE_SELECT_KEY.TAB) break;
                    if (evt.ctrlKey || evt.altKey || evt.shiftKey || this.selectedItem.value == '') break;
                    evt.preventDefault();
                    evt.stopImmediatePropagation();
                    if (!this.selectedItem.isSelectable) break;
