...
function populateDebuggerRuleList(container) {
    ...
    let ruleTemplate = container.find('#regex_debugger_rule_template');

    if (!ruleTemplate.length) {
        ...
        const host = container.find('.regex-debugger-container').first();
        const target = host.length ? host : container;
        target.append(fallbackTemplateHtml);
        ruleTemplate = container.find('#regex_debugger_rule_template');
    }
