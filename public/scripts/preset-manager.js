...
import { delegatePanelEvent, PANEL_ROOTS } from './util/panel-delegation.js';
import { defineSaveScheduler, createSaveScheduler } from './util/save-scheduler.js';

const presetManagers = {};

defineSaveScheduler('preset-manager-select', {
    panelId: 'ai_response_configuration',
    triggers: ['change'],
    strategy: 'raf',
});

const queuePresetSelectChange = createSaveScheduler('preset-manager-select', () => {
    let scheduled = false;
        ...
        requestAnimationFrame(() => {
            ...
            pending.forEach((element) => {
                try {
                    ...
                    console.warn('[PresetManager] 调度 change 事件失败', error);
                }
            });
            pending.clear();
        });
...
const PRESET_PANEL_ROOTS = `${PANEL_ROOTS.LEFT_NAV_ROOT}, ${PANEL_ROOTS.ADVANCED_FORMATTING_ROOT}`;

function delegatePresetEvent(eventType, selector, handler, namespace) {
    delegatePanelEvent(eventType, selector, handler, { root: PRESET_PANEL_ROOTS, namespace });
}
...
function autoSelectPreset() {
    ...
    if (!presetManager) {
        console.debug(`未找到 API ${main_api} 的预设管理器`);
        return;
    }
    ...
    if (!name) {
        console.debug(`未能为 API ${main_api} 找到可用的预设候选`);
        return;
    }
    ...
    if (preset === selectedPreset) {
        console.debug(`API ${main_api} 已选中预设：${name}`);
        return;
    }

    if (preset !== undefined && preset !== null) {
        console.log(`为 API ${main_api} 找到预设：${name}`);
...
function registerPresetManagers() {
    $('select[data-preset-manager-for]').each((_, e) => {
        ...
        for (const apiId of forData.split(',')) {
            console.debug(`正在为 API ${apiId} 注册预设管理器`);
...
class PresetManager {
    ...
    selectPreset(value) {
        ...
        queuePresetSelectChange($select[0]);
    }
    ...
    async savePreset(name, settings, { skipUpdate = false } = {}) {
        ...
        if (skipUpdate) {
            console.debug(`预设 ${name} 已保存，但未刷新列表`);
            return;
        }
    ...
    async renamePreset(newName) {
        ...
        try {
            ...
            console.error('预设重命名失败', error);
            throw new Error('预设重命名失败');
        }
    ...
    updateList(name, preset) {
        ...
        const isKeyed = this.isKeyedApi();
        const presetExists = isKeyed ? preset_names.includes(name) : Object.prototype.hasOwnProperty.call(preset_names, name);
        const $select = $(this.select);
        const selectEl = $select.length ? $select[0] : null;

        if (!selectEl) {
            console.warn('[PresetManager] 无法更新列表：缺少对应的 <select> 元素');
            return;
        }

        if (presetExists) {
            if (isKeyed) {
                const index = preset_names.indexOf(name);
                if (index !== -1) {
                    presets[index] = preset;
                }
                selectEl.value = name;
            } else {
                const mappedIndex = preset_names[name];
                if (mappedIndex !== undefined) {
                    presets[mappedIndex] = preset;
                    selectEl.value = String(mappedIndex);
                }
            }
        } else {
            const nextIndex = presets.push(preset) - 1;
            if (isKeyed) {
                preset_names[nextIndex] = name;
                const option = new Option(name, name);
                selectEl.add(option);
                selectEl.value = name;
            } else {
                preset_names[name] = nextIndex;
                const option = new Option(name, String(nextIndex));
                selectEl.add(option);
                selectEl.value = String(nextIndex);
            }
        }

        queuePresetSelectChange(selectEl);
    }
...
async function presetCommandCallback(_, name) {
    ...
    if (!name) {
        console.log('未为 /preset 命令提供名称，将沿用当前预设');
        return currentPreset;
    }

    if (!Array.isArray(allPresets) || allPresets.length === 0) {
        console.log(`API ${main_api} 没有可用预设`);
        return currentPreset;
    }
    ...
    if (exactMatch) {
        console.log('找到完全匹配的预设', exactMatch);
        ...
        if (fuzzyPresetValue) {
            console.log('找到模糊匹配的预设', fuzzyPresetName);
...
export async function initPresetManager() {
    ...
    delegatePresetEvent('click', '[data-preset-manager-update]', async function () {
        const apiId = $(this).data('preset-manager-update');
        const presetManager = getPresetManager(apiId);

        if (!presetManager) {
            console.warn(`未找到 API ${apiId} 的预设管理器`);
            return;
        }
    ...
    delegatePresetEvent('click', '[data-preset-manager-new]', async function () {
    ...
    delegatePresetEvent('click', '[data-preset-manager-rename]', async function () {
        ...
        if (!newName || oldName === newName) {
            console.debug(!presetManager.isAdvancedFormatting() ? '预设重命名已取消' : '模板重命名已取消');
            return;
        }
        if (equalsIgnoreCaseAndAccents(oldName, newName)) {
    ...
    delegatePresetEvent('click', '[data-preset-manager-export]', async function () {
    ...
    delegatePresetEvent('click', '[data-preset-manager-import]', async function () {
        const apiId = $(this).data('preset-manager-import');
        $(`[data-preset-manager-file="${apiId}"]`).trigger('click');
    }, 'presetImportTrigger');

    delegatePresetEvent('change', '[data-preset-manager-file]', async function (e) {
    ...
    delegatePresetEvent('click', '[data-preset-manager-delete]', async function () {
    ...
    delegatePresetEvent('click', '[data-preset-manager-restore]', async function () {
