...
function onBackgroundFilterInput(value) {
    const filterValue = String(value ?? '').toLowerCase();
    $('#bg_menu_content > div').each(function () {
        const $bgContent = $(this);
        const title = String($bgContent.attr('title') ?? '').toLowerCase();
        $bgContent.toggle(title.includes(filterValue));
    });
}

const onBackgroundFilterInputDebounced = debounce(onBackgroundFilterInput, 200);
...
export function initBackgrounds() {
    ...
    $('#bg-filter').on('input', function () {
        onBackgroundFilterInputDebounced($(this).val());
    });
