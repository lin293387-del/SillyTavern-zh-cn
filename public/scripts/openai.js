...
export function getChatCompletionModel(source = null) {
    ...
    switch (activeSource) {
        ...
        default:
            console.error(`未知的聊天补全来源：${activeSource}`);
            return '';
    }
}
...
class Message {
    ...
    constructor(role, content, identifier) {
        ...
        if (!this.role) {
            console.log(`消息角色未设置，已为标识符 '${this.identifier}' 默认为 'system'`);
            this.role = 'system';
        }
    ...
    async addImage(image) {
        ...
        try {
            ...
            console.error('无法获取图像的 token 成本', error);
        }
    }
    ...
    async addVideo(video) {
        ...
        try {
            ...
            this.tokens += 10000;
            console.error('无法获取视频的 token 成本', error);
        }
    }
...
async function getStatusOpen() {
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.CUSTOM && !isValidUrl(oai_settings.custom_url)) {
        console.debug('自定义 OpenAI API 终端地址无效：', oai_settings.custom_url);
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.AZURE_OPENAI && !isValidUrl(oai_settings.azure_base_url)) {
        console.debug('Azure OpenAI API 终端地址无效：', oai_settings.azure_base_url);
    ...
    try {
        ...
        if (responseText) {
            try {
                ...
                console.warn('[OpenAI] 无法解析状态检查响应为 JSON', parseError);
            }
        }
        ...
        if (!response.ok) {
            ...
            console.warn('[OpenAI] 状态检查失败:', errorMessage);
            if (!canBypass) {
...
function onLogitBiasPresetChange() {
    ...
    list.sortable({
        ...
        stop: function () {
            ...
            console.log('Logit 偏置已重新排序：', preset);
            saveSettingsDebounced();
        },
    });
...
async function onPresetImportFileChange(e) {
    ...
    if (shouldConfirm) {
        const textHeader = '导入的预设包含代理和/或自定义终端配置。';
        const textMessage = fields.join('<br>');
        const cancelButton = { text: '取消导入', result: POPUP_RESULT.CANCELLED, appendAtEnd: true };
        const popupOptions = { customButtons: [cancelButton], okButton: '移除这些设置', cancelButton: '保留原样导入' };
        const popupResult = await Popup.show.confirm(textHeader, textMessage, popupOptions);

        if (popupResult === POPUP_RESULT.CANCELLED) {
            console.log('导入操作已被用户取消');
            return;
        }
    ...
    if (name in openai_setting_names) {
        const confirm = await callGenericPopup('预设名称已存在，是否覆盖？', POPUP_TYPE.CONFIRM);
...
async function onExportPresetClick() {
    ...
    const presetKey = openai_setting_names?.[presetName];
    const presetSource = presetKey !== undefined ? openai_settings?.[presetKey] : null;
    if (!presetSource) {
        console.warn('[OpenAI] 导出时未找到预设', { presetName, presetKey });
        toastr.error(t`Preset not found`);
        return;
    }
    ...
    if (fieldValues.length > 0) {
        ...
        const cancelButton = { text: '取消', result: POPUP_RESULT.CANCELLED, appendAtEnd: true };
        const popupOptions = { customButtons: [cancelButton] };
        const popupResult = await Popup.show.confirm(textHeader, textMessage, popupOptions);

        if (popupResult === POPUP_RESULT.CANCELLED) {
            console.log('导出操作已被用户取消');
            return;
        }
...
async function onDeletePresetClick() {
    ...
    if (response.status === 404) {
        toastr.info(t`Preset already removed from server`);
    } else {
    ...
    const remainingPresetNames = Object.keys(openai_setting_names);
    if (remainingPresetNames.length) {
        oai_settings.preset_settings_openai = remainingPresetNames[0];
...
function onSettingsPresetChange() {
    ...
    const presetKey = openai_setting_names?.[oai_settings.preset_settings_openai];
    const presetSource = presetKey ? openai_settings?.[presetKey] : undefined;
    if (!presetSource) {
        console.warn('未找到 OpenAI 预设，跳过应用设置', { presetName, presetKey });
    }
    const preset = presetSource ? structuredClone(presetSource) : {};
...
async function onModelChange() {
    ...
    if ($(this).is('#model_claude_select')) {
        ...
        console.log('Claude 模型已切换为', value);
    ...
    if ($(this).is('#model_openai_select')) {
        console.log('OpenAI 模型已切换为', value);
        oai_settings.openai_model = value;
    }

    if ($(this).is('#model_openrouter_select')) {
        if (!value) {
            console.debug('未选择 OpenRouter 模型，已忽略。');
            return;
        }

        console.log('OpenRouter 模型已切换为', value);
...
async function onConnectButtonClick(e) {
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.OPENROUTER) {
        ...
        if (!secret_state[SECRET_KEYS.OPENROUTER]) {
            console.log('未检测到 OpenRouter 的密钥');
            return;
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.MAKERSUITE) {
        ...
        if (!secret_state[SECRET_KEYS.MAKERSUITE] && !oai_settings.reverse_proxy) {
            console.log('未检测到 Google AI Studio 的密钥');
            return;
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.VERTEXAI) {
        if (oai_settings.vertexai_auth_mode === 'express') {
            ...
            if (!secret_state[SECRET_KEYS.VERTEXAI] && !oai_settings.reverse_proxy) {
                console.log('未检测到 Vertex AI Express 模式的密钥');
                return;
            }
        } else {
            ...
            if (!secret_state[SECRET_KEYS.VERTEXAI_SERVICE_ACCOUNT]) {
                toastr.error('Vertex AI 完整版需要 Service Account JSON，请确认内容并保存。');
                return;
            }
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.MISTRALAI) {
        ...
        if (!secret_state[SECRET_KEYS.MISTRALAI] && !oai_settings.reverse_proxy) {
            console.log('未检测到 MistralAI 的密钥');
            return;
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.PERPLEXITY) {
        ...
        if (!secret_state[SECRET_KEYS.PERPLEXITY]) {
            console.log('未检测到 Perplexity 的密钥');
            return;
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.ELECTRONHUB) {
        ...
        if (!secret_state[SECRET_KEYS.ELECTRONHUB]) {
            console.log('未检测到 Electron Hub 的密钥');
            return;
        }
    }
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.AIMLAPI) {
        ...
        if (!secret_state[SECRET_KEYS.AIMLAPI]) {
            console.log('未检测到 AI/ML API 的密钥');
            return;
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.FIREWORKS) {
        ...
        if (!secret_state[SECRET_KEYS.FIREWORKS]) {
            console.log('未检测到 Fireworks 的密钥');
            return;
        }
    }
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.AZURE_OPENAI) {
        ...
        if (!api_key_azure_openai && !secret_state[SECRET_KEYS.AZURE_OPENAI]) {
            console.log('未检测到 Azure OpenAI 的密钥');
            return;
        }
    }
