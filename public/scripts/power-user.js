...
const STORY_TEMPLATE_CACHE_LIMIT = 32;
/** @type {Map<string, any>} */
const storyTemplateCache = new Map();

function getCompiledStoryTemplate(storyString) {
    if (storyTemplateCache.has(storyString)) {
        const cached = storyTemplateCache.get(storyString);
        // Touch for simple LRU behaviour
        storyTemplateCache.delete(storyString);
        storyTemplateCache.set(storyString, cached);
        return cached;
    }

    const compiled = Handlebars.compile(storyString, { noEscape: true });
    storyTemplateCache.set(storyString, compiled);
    if (storyTemplateCache.size > STORY_TEMPLATE_CACHE_LIMIT) {
        const oldestKey = storyTemplateCache.keys().next().value;
        storyTemplateCache.delete(oldestKey);
    }
    return compiled;
}
...
import { countOccurrences, debounce, delay, download, getFileText, getSanitizedFilename, getStringHash, isOdd, isTrueBoolean, onlyUnique, resetScrollHeight, shuffle, sortMoments, stringToRange, timestampToMoment, cancelDebounce, throttle } from './utils.js';
...
export const power_user = {
    ...
    offscreen_optimization: true,
    extension_debug_panel: false,
...
export function hydrateSmartThemeDefaults(force = false) {
    const root = document.documentElement;
    if (!root) {
        return;
    }
    const computed = getComputedStyle(root);
    for (const [key, cssVar] of Object.entries(smartThemeVariableMap)) {
        const currentValue = power_user[key];
        if (!force && typeof currentValue === 'string' && currentValue.trim()) {
            continue;
        }
        const nextValue = computed.getPropertyValue(cssVar)?.trim();
        if (nextValue) {
            power_user[key] = nextValue;
        }
    }
}

if (document.readyState === 'loading') {
    window.addEventListener('DOMContentLoaded', () => hydrateSmartThemeDefaults(false), { once: true });
} else {
    hydrateSmartThemeDefaults(false);
}
...
function createStatefulThrottle(intervalMs) {
    const timestamps = new Map();
    return (stateKey = 'default') => {
        const now = Date.now();
        const lastShown = timestamps.get(stateKey) ?? 0;
        if (now - lastShown >= intervalMs) {
            timestamps.set(stateKey, now);
            return true;
        }
        return false;
    };
...
function toggleClassOnTarget(target, className, shouldAdd) {
    const resolvedTarget = typeof target === 'function' ? target() : target;
    if (!resolvedTarget) {
        return;
    }

    if (resolvedTarget.jquery) {
        resolvedTarget.toggleClass(className, shouldAdd);
        return;
    }
    ...
    if (resolvedTarget instanceof Element || (typeof HTMLElement !== 'undefined' && resolvedTarget instanceof HTMLElement)) {
        resolvedTarget.classList.toggle(className, shouldAdd);
        return;
    }

    if (resolvedTarget instanceof NodeList || Array.isArray(resolvedTarget)) {
        $(resolvedTarget).toggleClass(className, shouldAdd);
        return;
    }
...
function applyToggle(moduleKey, toggleKey, ...args) {
    const module = toggleModules[moduleKey];
    if (!module) {
        return false;
    }

    const config = module[toggleKey];
    if (!config) {
        return false;
    }

    const rawValue = typeof config.valueGetter === 'function'
        ? config.valueGetter(...args)
        : (config.valueKey !== undefined ? getNestedValue(power_user, config.valueKey) : config.value);
    const boolValue = Boolean(rawValue);

    if (config.checkbox) {
        const checkboxTarget = typeof config.checkbox === 'function' ? config.checkbox() : config.checkbox;
        $(checkboxTarget).prop(config.checkboxProp ?? 'checked', boolValue);
    }

    if (Array.isArray(config.classToggles)) {
        for (const classToggle of config.classToggles) {
            const target = typeof classToggle.target === 'function' ? classToggle.target() : classToggle.target;
            const shouldAdd = classToggle.invert ? !boolValue : boolValue;
            toggleClassOnTarget(target, classToggle.className, shouldAdd);
        }
    }

    if (typeof config.afterToggle === 'function') {
        config.afterToggle(boolValue, ...args);
    }

    return boolValue;
}

const workerOptimizationToastLimiter = createStatefulThrottle(1000);

let virtualizationWarningTimestamp = 0;
const VIRTUALIZATION_WARNING_INTERVAL_MS = 1000;

const toggleModules = {
    theme: {
        hotswap: {
            valueKey: 'hotswap_enabled',
            checkbox: '#hotswapEnabled',
            classToggles: [{ target: () => document.body, className: 'no-hotswap', invert: true }],
        },
        timer: {
            valueKey: 'timer_enabled',
            checkbox: '#messageTimerEnabled',
            classToggles: [{ target: () => document.body, className: 'no-timer', invert: true }],
        },
        timestamps: {
            valueKey: 'timestamps_enabled',
            checkbox: '#messageTimestampsEnabled',
            classToggles: [{ target: () => document.body, className: 'no-timestamps', invert: true }],
        },
        icons: {
            valueKey: 'timestamp_model_icon',
            checkbox: '#messageModelIconEnabled',
            classToggles: [{ target: () => document.body, className: 'no-modelIcons', invert: true }],
        },
        tokenCount: {
            valueKey: 'message_token_count_enabled',
            checkbox: '#messageTokensEnabled',
            classToggles: [{ target: () => document.body, className: 'no-tokenCount', invert: true }],
        },
        mesIdDisplay: {
            valueKey: 'mesIDDisplay_enabled',
            checkbox: '#mesIDDisplayEnabled',
            classToggles: [{ target: () => document.body, className: 'no-mesIDDisplay', invert: true }],
        },
        hideChatAvatars: {
            valueKey: 'hideChatAvatars_enabled',
            checkbox: '#hideChatAvatarsEnabled',
            classToggles: [{ target: () => document.body, className: 'hideChatAvatars' }],
        },
        messageActions: {
            valueKey: 'expand_message_actions',
            checkbox: '#expandMessageActions',
            classToggles: [{ target: () => document.body, className: 'expandMessageActions' }],
            afterToggle: () => $('.extraMesButtons, .extraMesButtonsHint').removeAttr('style'),
        },
    },
    chat: {
        compactInputArea: {
            valueKey: 'compact_input_area',
            checkbox: '#compact_input_area',
            classToggles: [{ target: () => $('#send_form'), className: 'compact' }],
        },
        swipeAllMessages: {
            valueKey: 'show_swipe_num_all_messages',
            checkbox: '#show_swipe_num_all_messages',
            classToggles: [{ target: () => document.body, className: 'swipeAllMessages' }],
        },
    },
    performance: {
        virtualization: {
            valueKey: 'enable_virtualization',
            checkbox: '#enable_virtualization',
            classToggles: [{ target: () => document.body, className: 'virtualization-disabled', invert: true }],
            afterToggle: (enabled, options = {}) => {
                const showWarning = options?.showWarning ?? true;
                if (!enabled && showWarning) {
                    const now = Date.now();
                    if (now - virtualizationWarningTimestamp >= VIRTUALIZATION_WARNING_INTERVAL_MS) {
                        virtualizationWarningTimestamp = now;
                        toastr.warning('已禁用虚拟化，长聊天或大型列表的响应速度可能明显下降（建议刷新页面）。', '性能警告', { timeOut: 3000 });
                    }
                }
            },
        },
        workerOptimization: {
            valueKey: 'worker_optimization',
            checkbox: '#enable_worker',
            afterToggle: (enabled, options = {}) => {
                const showToast = options?.showToast ?? false;
                setWorkerOptimizationEnabled(enabled);
                if (!showToast) {
                    return;
                }
                const stateKey = enabled ? 'enabled' : 'disabled';
                if (!workerOptimizationToastLimiter(stateKey)) {
                    return;
                }
                const message = enabled
                    ? '已启用 Worker 优化。请刷新页面以确保设置生效。'
                    : '已关闭 Worker 优化。请刷新页面以确保设置生效。';
                toastr.success(message, undefined, { timeOut: 3000 });
            },
        },
        extensionDebugPanel: {
            valueKey: 'extension_debug_panel',
            checkbox: '#enable_extension_debug_panel',
            afterToggle: (enabled) => {
                const root = globalThis.SillyTavern ?? (globalThis.SillyTavern = {});
                const extensions = root.extensions ?? (root.extensions = {});
                if (typeof extensions.debugPanel?.setEnabled === 'function') {
                    extensions.debugPanel.setEnabled(enabled);
                }
            },
        },
        reducedMotion: {
            valueKey: 'reduced_motion',
            checkbox: '#reduced_motion',
            classToggles: [{ target: () => document.body, className: 'reduced-motion' }],
            afterToggle: (enabled) => {
                jQuery.fx.off = enabled;
                const overrideDuration = enabled ? 0 : ANIMATION_DURATION_DEFAULT;
...
const VIRTUALIZATION_TOGGLE_DEBOUNCE_MS = 120;
const emitVirtualizationToggleImmediate = (enabled) => {
    window.dispatchEvent(new CustomEvent('virtualization-toggle', { detail: { enabled } }));
};
const emitVirtualizationToggleDebounced = debounce(emitVirtualizationToggleImmediate, VIRTUALIZATION_TOGGLE_DEBOUNCE_MS);

function notifyVirtualizationToggle(enabled, { immediate = false } = {}) {
    if (immediate) {
        cancelDebounce(emitVirtualizationToggleImmediate);
        emitVirtualizationToggleImmediate(enabled);
        return;
    }

    emitVirtualizationToggleDebounced(enabled);
}

function switchHotswap() {
    applyToggle('theme', 'hotswap');
}

function switchTimer() {
    applyToggle('theme', 'timer');
}

function switchTimestamps() {
    applyToggle('theme', 'timestamps');
}

function switchIcons() {
    applyToggle('theme', 'icons');
}

function switchTokenCount() {
    applyToggle('theme', 'tokenCount');
}

function switchMesIDDisplay() {
    applyToggle('theme', 'mesIdDisplay');
}

function switchHideChatAvatars() {
    applyToggle('theme', 'hideChatAvatars');
}

function switchMessageActions() {
    applyToggle('theme', 'messageActions');
}

function switchVirtualization(showWarning = true) {
    applyToggle('performance', 'virtualization', { showWarning });
}

function switchWorkerOptimization(showToast = false) {
    applyToggle('performance', 'workerOptimization', { showToast });
}

function switchExtensionDebugPanel() {
    applyToggle('performance', 'extensionDebugPanel');
}

function switchReducedMotion() {
    applyToggle('performance', 'reducedMotion');
}

function switchCompactInputArea() {
    applyToggle('chat', 'compactInputArea');
}

function switchSwipeNumAllMessages() {
    applyToggle('chat', 'swipeAllMessages');
}
...
async function switchLabMode({ noReset = false } = {}) {
    ...
    /*     if (power_user.enableZenSliders && power_user.enableLabMode) {
            toastr.warning("Zen 滑块启用时无法开启 Lab Mode")
            return
                    ...
                    .on('mouseup blur', function () {
                        ...
                        if (isManualInput) {
                            ...
                            if (manualInput >= sliderMin && manualInput <= sliderMax) {
                                ...
                                toastr.warning(`输入无效，请输入 ${sliderMin} 至 ${sliderMax} 之间的值`);
                                console.log(valueBeforeManualInput);
...
function applyChatDisplay() {
    if ([null, undefined].includes(power_user.chat_display)) {
        console.debug('applyChatDisplay：未检测到聊天展示类型定义');
        power_user.chat_display = chat_styles.DEFAULT;
    }
    console.debug(`power_user.chat_display 当前值 ${power_user.chat_display}`);
    $('#chat_display').val(power_user.chat_display).prop('selected', true);

    switch (power_user.chat_display) {
        case 0: {
            console.debug('applyChatDisplay：应用默认聊天样式');
            $('body').removeClass('bubblechat');
            $('body').removeClass('documentstyle');
            break;
        }
        case 1: {
            console.debug('applyChatDisplay：应用气泡聊天样式');
            $('body').addClass('bubblechat');
            $('body').removeClass('documentstyle');
            break;
        }
        case 2: {
            console.debug('applyChatDisplay：应用文档风格');
            $('body').removeClass('bubblechat');
...
function applyToastrPosition() {
    if (!toastPositionClasses.includes(power_user.toastr_position)) {
        ...
        console.warn(`applyToastrPosition：检测到无效通知位置，已回退为 ${defaultToastPosition}`);
    }
...
function applyTheme(name) {
    ...
    const themeProperties = [
        ...
        },
        {
            ...
            action: () => {
                switchVirtualization(false);
                notifyVirtualizationToggle(power_user.enable_virtualization, { immediate: true });
            },
        },
        {
            key: 'extension_debug_panel',
            action: () => {
                switchExtensionDebugPanel();
            },
        },
        {
            key: 'worker_optimization',
            action: () => {
                switchWorkerOptimization(false);
            },
        },
        {
            key: 'reduced_motion',
    ...
    for (const { key, selector, type, action } of themeProperties) {
        if (theme[key] !== undefined) {
            ...
            console.debug(`主题字段 ${key} 为空，已跳过`);
        }
    }

    console.log('已应用主题：' + name);
}
...
async function applyMovingUIPreset(name) {
    ...
    console.log('已应用 MovingUI 预设：' + name);
    loadMovingUIState();
    saveSettingsDebounced();
}
...
export function applyStylePins() {
    try {
        ...
        console.error('应用样式固定点时出错：', error);
    }
}
...
function applyControlBindings(bindings) {
    for (const binding of bindings) {
        ...
        const rawValue = valueKey !== undefined ? getNestedValue(power_user, valueKey) : (typeof value === 'function' ? value() : value);
        const resolvedValue = typeof transform === 'function' ? transform(rawValue, binding) : rawValue;

        switch (type) {
            case 'checkbox': {
                $elements.prop(propName, !!resolvedValue);
                break;
            }
            case 'text': {
                $elements.text(resolvedValue ?? '');
                break;
            }
            case 'attr': {
                if (!attr) {
                ...
                if (resolvedValue === undefined || resolvedValue === null) {
                    ...
                    $elements.attr(attr, resolvedValue);
                }
                break;
            }
            case 'radioGroup': {
                ...
                if (resolvedValue !== undefined && resolvedValue !== null) {
                    const escaped = escapeSelectorValue(resolvedValue);
                    $elements.filter(`[value="${escaped}"]`).prop('checked', true);
                }
                break;
            }
            case 'custom': {
                // Intentionally noop; handled by after/apply hooks.
                break;
            }
            default: {
                const valueToSet = resolvedValue ?? '';
                $elements.val(valueToSet);
                break;
            }
        }
        ...
        if (Array.isArray(triggers)) {
            for (const trigger of triggers) {
                ...
                if (trigger.data !== undefined) {
                    $elements.trigger(trigger.event, trigger.data);
                } else {
                    $elements.trigger(trigger.event);
                }
            }
        }

        if (typeof after === 'function') {
            after($elements, resolvedValue, rawValue);
        }
    }
}
...
export async function loadPowerUserSettings(settings, data) {
    ...
    const toStringOrEmpty = (input) => (input === undefined || input === null ? '' : String(input));
    const arrayToList = (input) => (Array.isArray(input) ? input.join(', ') : (input ?? ''));

    const controlBindings = [
        // General toggles
        { selector: '#single_line', type: 'checkbox', valueKey: 'single_line' },
        { selector: '#relaxed_api_urls', type: 'checkbox', valueKey: 'relaxed_api_urls' },
        { selector: '#world_import_dialog', type: 'checkbox', valueKey: 'world_import_dialog' },
        { selector: '#enable_auto_select_input', type: 'checkbox', valueKey: 'enable_auto_select_input' },
        { selector: '#enable_md_hotkeys', type: 'checkbox', valueKey: 'enable_md_hotkeys' },
        { selector: '#trim_spaces', type: 'checkbox', valueKey: 'trim_spaces' },
        { selector: '#continue_on_send', type: 'checkbox', valueKey: 'continue_on_send' },
        {
            selector: '#quick_continue',
            type: 'checkbox',
            valueKey: 'quick_continue',
            after: (_, value) => $('#mes_continue').css('display', value ? '' : 'none'),
        },
        {
            selector: '#quick_impersonate',
            type: 'checkbox',
            value: () => power_user.quick_continue,
        },
        { selector: '#gestures-checkbox', type: 'checkbox', valueKey: 'gestures' },
        { selector: '#auto_swipe', type: 'checkbox', valueKey: 'auto_swipe' },
        { selector: '#auto_swipe_minimum_length', type: 'value', valueKey: 'auto_swipe_minimum_length', transform: toStringOrEmpty },
        {
            selector: '#auto_swipe_blacklist',
            type: 'value',
            valueKey: 'auto_swipe_blacklist',
            transform: arrayToList,
        },
        { selector: '#auto_swipe_blacklist_threshold', type: 'value', valueKey: 'auto_swipe_blacklist_threshold', transform: toStringOrEmpty },
        { selector: '#custom_stopping_strings', type: 'text', valueKey: 'custom_stopping_strings' },
        { selector: '#custom_stopping_strings_macro', type: 'checkbox', valueKey: 'custom_stopping_strings_macro' },
        { selector: '#fuzzy_search_checkbox', type: 'checkbox', valueKey: 'fuzzy_search' },
        { selector: '#persona_show_notifications', type: 'checkbox', valueKey: 'persona_show_notifications' },
        { selector: '#persona_allow_multi_connections', type: 'checkbox', valueKey: 'persona_allow_multi_connections' },
        { selector: '#persona_auto_lock', type: 'checkbox', valueKey: 'persona_auto_lock' },
        { selector: '#encode_tags', type: 'checkbox', valueKey: 'encode_tags' },
        {
            selector: '#example_messages_behavior',
            type: 'value',
            value: getExampleMessagesBehavior,
            after: ($elements, value) => {
                const escaped = escapeSelectorValue(value);
                $elements.find('option').prop('selected', false);
                $elements.find(`option[value="${escaped}"]`).prop('selected', true);
            },
        },
        {
            selector: '#instruct_derived',
            type: 'checkbox',
            value: () => !!power_user.instruct_derived,
            after: ($elements, value) => $elements.parent().find('i').toggleClass('toggleEnabled', value),
        },
        {
            selector: '#context_derived',
            type: 'checkbox',
            value: () => !!power_user.context_derived,
            after: ($elements, value) => $elements.parent().find('i').toggleClass('toggleEnabled', value),
        },
        { selector: '#context_size_derived', type: 'checkbox', value: () => !!power_user.context_size_derived },

        // Logging & queue toggles
        { selector: '#console_log_prompts', type: 'checkbox', valueKey: 'console_log_prompts' },
        { selector: '#request_token_probabilities', type: 'checkbox', valueKey: 'request_token_probabilities' },
        { selector: '#show_group_chat_queue', type: 'checkbox', valueKey: 'show_group_chat_queue' },
        { selector: '#auto_fix_generated_markdown', type: 'checkbox', valueKey: 'auto_fix_generated_markdown' },
        { selector: '#auto_scroll_chat_to_bottom', type: 'checkbox', valueKey: 'auto_scroll_chat_to_bottom' },
        { selector: '#bogus_folders', type: 'checkbox', valueKey: 'bogus_folders' },
        { selector: '#zoomed_avatar_magnification', type: 'checkbox', valueKey: 'zoomed_avatar_magnification' },
        ...
        {
            selector: '#send_on_enter',
            type: 'value',
            valueKey: 'send_on_enter',
            transform: toStringOrEmpty,
            triggers: [{ event: 'change' }],
        },
        {
            selector: '#confirm_message_delete',
            type: 'checkbox',
            value: () => (power_user.confirm_message_delete !== undefined ? !!power_user.confirm_message_delete : true),
        },
        { selector: '#spoiler_free_mode', type: 'checkbox', valueKey: 'spoiler_free_mode' },
        { selector: '#collapse-newlines-checkbox', type: 'checkbox', valueKey: 'collapse_newlines' },
        { selector: '#always-force-name2-checkbox', type: 'checkbox', valueKey: 'always_force_name2' },
        { selector: '#trim_sentences_checkbox', type: 'checkbox', valueKey: 'trim_sentences' },
        { selector: '#disable_group_trimming', type: 'checkbox', valueKey: 'disable_group_trimming' },
        { selector: '#markdown_escape_strings', type: 'value', valueKey: 'markdown_escape_strings' },
        { selector: '#fast_ui_mode', type: 'checkbox', valueKey: 'fast_ui_mode' },
        { selector: '#waifuMode', type: 'checkbox', valueKey: 'waifuMode' },
        { selector: '#movingUImode', type: 'checkbox', valueKey: 'movingUI' },
        { selector: '#noShadowsmode', type: 'checkbox', valueKey: 'noShadows' },
        { selector: '#start_reply_with', type: 'text', valueKey: 'user_prompt_bias' },
        { selector: '#chat-show-reply-prefix-checkbox', type: 'checkbox', valueKey: 'show_user_prompt_bias' },
        { selector: '#auto_continue_enabled', type: 'checkbox', valueKey: 'auto_continue.enabled' },
        { selector: '#auto_continue_allow_chat_completions', type: 'checkbox', valueKey: 'auto_continue.allow_chat_completions' },
        { selector: '#auto_continue_target_length', type: 'value', valueKey: 'auto_continue.target_length', transform: toStringOrEmpty },
        { selector: '#play_message_sound', type: 'checkbox', valueKey: 'play_message_sound' },
        { selector: '#play_sound_unfocused', type: 'checkbox', valueKey: 'play_sound_unfocused' },
        { selector: '#never_resize_avatars', type: 'checkbox', valueKey: 'never_resize_avatars' },
        { selector: '#show_card_avatar_urls', type: 'checkbox', valueKey: 'show_card_avatar_urls' },
        { selector: '#auto_save_msg_edits', type: 'checkbox', valueKey: 'auto_save_msg_edits' },
        { selector: '#allow_name1_display', type: 'checkbox', valueKey: 'allow_name1_display' },
        { selector: '#allow_name2_display', type: 'checkbox', valueKey: 'allow_name2_display' },
        { selector: '#hotswapEnabled', type: 'checkbox', valueKey: 'hotswap_enabled' },
        { selector: '#messageTimerEnabled', type: 'checkbox', valueKey: 'timer_enabled' },
        { selector: '#messageTimestampsEnabled', type: 'checkbox', valueKey: 'timestamps_enabled' },
        { selector: '#messageModelIconEnabled', type: 'checkbox', valueKey: 'timestamp_model_icon' },
        { selector: '#mesIDDisplayEnabled', type: 'checkbox', valueKey: 'mesIDDisplay_enabled' },
        { selector: '#hideChatAvatarsEnabled', type: 'checkbox', valueKey: 'hideChatAvatars_enabled' },
        { selector: '#prefer_character_prompt', type: 'checkbox', valueKey: 'prefer_character_prompt' },
        { selector: '#prefer_character_jailbreak', type: 'checkbox', valueKey: 'prefer_character_jailbreak' },
        {
            selector: '#enableZenSliders',
            type: 'checkbox',
            valueKey: 'enableZenSliders',
            triggers: [{ event: 'input' }],
        },
        {
            selector: '#enableLabMode',
            type: 'checkbox',
            valueKey: 'enableLabMode',
            triggers: [{ event: 'input', data: { fromInit: true } }],
        },
        { selector: 'input[name="avatar_style"]', type: 'radioGroup', valueKey: 'avatar_style' },
        {
            selector: '#chat_display',
            type: 'value',
            valueKey: 'chat_display',
            transform: toStringOrEmpty,
            triggers: [{ event: 'change' }],
        },
        {
            selector: '#toastr_position',
            type: 'value',
            valueKey: 'toastr_position',
            triggers: [{ event: 'change' }],
        },
        { selector: '#chat_width_slider', type: 'value', valueKey: 'chat_width', transform: toStringOrEmpty },
        { selector: '#token_padding', type: 'value', valueKey: 'token_padding', transform: toStringOrEmpty },
        { selector: '#aux_field', type: 'value', valueKey: 'aux_field', transform: toStringOrEmpty },
        { selector: '#tag_import_setting', type: 'value', valueKey: 'tag_import_setting', transform: toStringOrEmpty },

        // Stscript bindings
        {
            selector: '#stscript_autocomplete_state',
            type: 'value',
            valueKey: 'stscript.autocomplete.state',
            transform: (value) => value ?? defaultStscript.autocomplete.state,
            triggers: [{ event: 'input' }],
        },
        {
            selector: '#stscript_autocomplete_autoHide',
            type: 'checkbox',
            value: () => power_user.stscript.autocomplete.autoHide ?? false,
            triggers: [{ event: 'input' }],
        },
        {
            selector: '#stscript_matching',
            type: 'value',
            valueKey: 'stscript.matching',
            transform: (value) => value ?? 'fuzzy',
        },
        {
            selector: '#stscript_autocomplete_style',
            type: 'value',
            valueKey: 'stscript.autocomplete.style',
            transform: (value) => value ?? 'theme',
            after: (_, value) => document.body.setAttribute('data-stscript-style', value ?? 'theme'),
        },
        {
            selector: '#stscript_autocomplete_select',
            type: 'value',
            valueKey: 'stscript.autocomplete.select',
            transform: (value) => value ?? (AUTOCOMPLETE_SELECT_KEY.TAB + AUTOCOMPLETE_SELECT_KEY.ENTER),
        },
        {
            selector: '#stscript_parser_flag_strict_escaping',
            type: 'checkbox',
            value: () => power_user.stscript.parser.flags?.[PARSER_FLAG.STRICT_ESCAPING] ?? false,
        },
        {
            selector: '#stscript_parser_flag_replace_getvar',
            type: 'checkbox',
            value: () => power_user.stscript.parser.flags?.[PARSER_FLAG.REPLACE_GETVAR] ?? false,
        },
        {
            selector: '#stscript_autocomplete_font_scale',
            type: 'value',
            value: () => power_user.stscript.autocomplete.font.scale ?? defaultStscript.autocomplete.font.scale,
            after: (_, value) => document.body.style.setProperty('--ac-font-scale', String(value ?? defaultStscript.autocomplete.font.scale)),
        },
        {
            selector: '#stscript_autocomplete_font_scale_counter',
            type: 'value',
            value: () => power_user.stscript.autocomplete.font.scale ?? defaultStscript.autocomplete.font.scale,
        },
        {
            selector: '#stscript_autocomplete_width_left',
            type: 'value',
            value: () => power_user.stscript.autocomplete.width.left ?? AUTOCOMPLETE_WIDTH.CHAT,
            transform: toStringOrEmpty,
            after: () => document.querySelector('#stscript_autocomplete_width_left')?.dispatchEvent(new Event('input', { bubbles: true })),
        },
        {
            selector: '#stscript_autocomplete_width_right',
            type: 'value',
            value: () => power_user.stscript.autocomplete.width.right ?? AUTOCOMPLETE_WIDTH.CHAT,
            transform: toStringOrEmpty,
            after: () => document.querySelector('#stscript_autocomplete_width_right')?.dispatchEvent(new Event('input', { bubbles: true })),
        },

        // Restore & counters
        { selector: '#restore_user_input', type: 'checkbox', valueKey: 'restore_user_input' },
        { selector: '#chat_truncation', type: 'value', valueKey: 'chat_truncation', transform: toStringOrEmpty },
        { selector: '#chat_truncation_counter', type: 'value', valueKey: 'chat_truncation', transform: toStringOrEmpty },
        { selector: '#streaming_fps', type: 'value', valueKey: 'streaming_fps', transform: toStringOrEmpty },
        { selector: '#streaming_fps_counter', type: 'value', valueKey: 'streaming_fps', transform: toStringOrEmpty },
        { selector: '#smooth_streaming', type: 'checkbox', valueKey: 'smooth_streaming' },
        { selector: '#smooth_streaming_speed', type: 'value', valueKey: 'smooth_streaming_speed', transform: toStringOrEmpty },
        { selector: '#font_scale', type: 'value', valueKey: 'font_scale', transform: toStringOrEmpty },
        { selector: '#font_scale_counter', type: 'value', valueKey: 'font_scale', transform: toStringOrEmpty },
        { selector: '#blur_strength', type: 'value', valueKey: 'blur_strength', transform: toStringOrEmpty },
        { selector: '#blur_strength_counter', type: 'value', valueKey: 'blur_strength', transform: toStringOrEmpty },
        { selector: '#shadow_width', type: 'value', valueKey: 'shadow_width', transform: toStringOrEmpty },
        { selector: '#shadow_width_counter', type: 'value', valueKey: 'shadow_width', transform: toStringOrEmpty },

        // Color pickers
        { selector: '#main-text-color-picker', type: 'attr', attr: 'color', valueKey: 'main_text_color' },
        { selector: '#italics-color-picker', type: 'attr', attr: 'color', valueKey: 'italics_text_color' },
        { selector: '#underline-color-picker', type: 'attr', attr: 'color', valueKey: 'underline_text_color' },
        { selector: '#quote-color-picker', type: 'attr', attr: 'color', valueKey: 'quote_text_color' },
        { selector: '#blur-tint-color-picker', type: 'attr', attr: 'color', valueKey: 'blur_tint_color' },
        { selector: '#chat-tint-color-picker', type: 'attr', attr: 'color', valueKey: 'chat_tint_color' },
        { selector: '#user-mes-blur-tint-color-picker', type: 'attr', attr: 'color', valueKey: 'user_mes_blur_tint_color' },
        { selector: '#bot-mes-blur-tint-color-picker', type: 'attr', attr: 'color', valueKey: 'bot_mes_blur_tint_color' },
        { selector: '#shadow-color-picker', type: 'attr', attr: 'color', valueKey: 'shadow_color' },
        { selector: '#border-color-picker', type: 'attr', attr: 'color', valueKey: 'border_color' },

        // Feature flags
        { selector: '#enable_virtualization', type: 'checkbox', valueKey: 'enable_virtualization' },
        { selector: '#enable_extension_debug_panel', type: 'checkbox', valueKey: 'extension_debug_panel' },
        { selector: '#enable_worker', type: 'checkbox', valueKey: 'worker_optimization' },
        { selector: '#reduced_motion', type: 'checkbox', valueKey: 'reduced_motion' },
        { selector: '#auto-connect-checkbox', type: 'checkbox', valueKey: 'auto_connect' },
        { selector: '#auto-load-chat-checkbox', type: 'checkbox', valueKey: 'auto_load_chat' },
        { selector: '#forbid_external_media', type: 'checkbox', valueKey: 'forbid_external_media' },
        { selector: '#pin_styles', type: 'checkbox', valueKey: 'pin_styles' },
        { selector: '#click_to_edit', type: 'checkbox', valueKey: 'click_to_edit' },

        // Custom handlers
        {
            selector: '#mes_impersonate',
            type: 'custom',
            valueKey: 'quick_impersonate',
            after: ($elements, value) => $elements.css('display', value ? '' : 'none'),
        },
    ];

    applyControlBindings(controlBindings);

    switchExtensionDebugPanel();

    const themesSelect = document.getElementById('themes');
    if (themesSelect) {
        const themesFragment = document.createDocumentFragment();
        ...
        for (const theme of themes) {
            ...
            themesFragment.appendChild(option);
        }
        themesSelect.appendChild(themesFragment);
    }

    const movingUIPresetsSelect = document.getElementById('movingUIPresets');
    if (movingUIPresetsSelect) {
        const presetsFragment = document.createDocumentFragment();
        movingUIPresetsSelect.innerHTML = '';
        for (const movingUIPreset of movingUIPresets) {
            ...
            presetsFragment.appendChild(option);
        }
        movingUIPresetsSelect.appendChild(presetsFragment);
    }
...
export function loadMovingUIState() {
    if (!isMobile()
        ...
        console.debug('正在加载 movingUI 状态');
        for (var elmntName of Object.keys(power_user.movingUIState)) {
            ...
            try {
                ...
                if (elmnt.length) {
                    console.debug(`加载 ${elmntName} 的状态`);
                    elmnt.css(elmntState);
                } else {
                    console.debug(`跳过 ${elmntName}，因为 DOM 中不存在对应元素`);
                }
            } catch (err) {
                console.debug(`处理 ${elmntName} 时出现错误：${err}`);
            }
        }
    } else {
        console.debug('跳过 movingUI 状态加载');
        return;
    }
}
...
async function loadContextSettings() {
    ...
    function autoFixStoryString(contextSettings) {
        ...
        function autoFixMissingField(field, position) {
            ...
            console.warn(`[Story String Validation] 故事字符串缺少字段 ${field}，将自动插入到 ${position} 位置。`);
    ...
    contextControls.forEach(control => {
        ...
        console.debug(`将 ${$element.prop('id')} 设置为 ${power_user.context[control.property]}`);
        ...
        $element.on('input', async function () {
            ...
            console.debug(`将 ${$element.prop('id')} 设置为 ${value}`);
...
export function renderStoryString(params, { customStoryString = null, customInstructSettings = null, customContextSettings = null } = {}) {
    try {
        ...
        const compiledTemplate = getCompiledStoryTemplate(storyString);
        ...
        toastr.error('请检查故事模板是否有效', '故事字符串渲染失败');
        console.error('故事字符串渲染失败', e);
...
function validateStoryString(storyString, params) {
    ...
    function validateMissingField(field, fallbackLegacyField = null) {
        ...
        if (!contains && params[field]) {
            ...
            console.warn(`故事字符串未包含 {{${field}}}，该字段已去除但存在内容：\n`, params[field]);
        }
    }
    ...
    if (fieldsToWarn.length > 0) {
        ...
        toastr.warning(`故事字符串缺少以下字段，但这些字段包含内容：${fieldsList}`, '故事字符串校验');
    }
...
async function deleteTheme() {
    ...
    if (!response.ok) {
        toastr.error('删除主题失败，请查看控制台了解详情。');
        return;
    }
...
async function importTheme(file) {
    ...
    if (typeof parsed.custom_css === 'string' && parsed.custom_css.includes('@import')) {
        ...
        const confirm = await callGenericPopup(template, POPUP_TYPE.CONFIRM);
        if (!confirm) {
            throw new Error('此主题包含@import语句');
        }
    }
    ...
    saveSettingsDebounced();
    toastr.success(parsed.name, '主题已导入');
}
...
async function saveTheme(name = undefined, theme = undefined) {
    ...
    if (!response.ok) {
        toastr.error('请检查服务器连接，必要时刷新页面以防数据丢失。', '主题保存失败');
        console.error('主题保存失败', response);
...
async function saveMovingUI() {
    ...
    if (response.ok) {
        ...
        toastr.error('保存 MovingUI 状态失败。');
        console.error('MovingUI 状态保存失败', response);
    }
}
...
async function resetMovablePanels(type) {
    ...
    eventSource.once(event_types.SETTINGS_UPDATED, () => {
        ...
        if (type === 'quiet') {
            ...
            toastr.warning('由于缩放或调整窗口，面板位置已重置');
...
async function loadUntilMesId(mesId) {
    ...
    if (!target.length) {
        toastr.error(`未找到 ID 为 ${mesId} 的消息`);
        return target;
    }
...
async function doDelMode(_, text) {
    ...
    if (count > chat.length) {
        toastr.warning(`最多只能删除 ${chat.length} 条消息。`);
        return '';
    }
...
async function setThemeCallback(_, themeName) {
    ...
    console.debug('主题模糊搜索结果：' + themeName, results);
    const theme = results[0]?.item;

    if (!theme) {
        toastr.warning(`未找到名称为 ${themeName} 的主题`);
        return;
    }
...
async function setmovingUIPreset(_, text) {
    ...
    console.debug('movingUI 预设模糊搜索结果：' + text, results);
    const preset = results[0]?.item;

    if (!preset) {
        toastr.warning(`未找到名称为 ${text} 的预设`);
        return;
    }
...
export function flushEphemeralStoppingStrings() {
    ...
    console.debug('清空临时停止字符串缓存：', EPHEMERAL_STOPPING_STRINGS);
...
export function getCustomStoppingStrings(limit = undefined) {
    function getPermanent() {
        try {
            ...
            console.warn('解析自定义停止字符串失败：', error);
            return [];
        }
    }
...
jQuery(() => {
    ...
    const reportZoomLevelDebounced = debounce(() => {
        ...
        console.log(`窗口尺寸变化：${coreTruthWinWidth}x${coreTruthWinHeight} -> ${window.innerWidth}x${window.innerHeight}`);
        console.debug(`缩放：${zoomLevel}，窗口：${winWidth}x${winHeight}，原始尺寸：${originalWidth}x${originalHeight} `);
        return zoomLevel;
    });
        ...
        if (Object.keys(power_user.movingUIState).length > 0) {
            for (var elmntName of Object.keys(power_user.movingUIState)) {
                ...
                try {
                    var elmnt = $('#' + $.escapeSelector(elmntName));
                    if (elmnt.length) {
                        console.log(`按 ${scaleX}x${scaleY} 缩放 ${elmntName} 至 ${newWidth}x${newHeight}`);
                        ...
                        console.log(`跳过 ${elmntName}，因为 DOM 中不存在对应元素`);
                    }
                } catch (err) {
                    console.log(`处理 ${elmntName} 时出现错误：${err}`);
                }
            }
        } else {
            console.debug('取消 MUI 重置', Object.keys(power_user.movingUIState).length);
        }
        saveSettingsDebounced();
    ...
    $('#example_messages_behavior').on('change', function () {
        ...
        console.log('将示例消息行为设置为', selectedOption);
        ...
        console.debug('power_user.pin_examples 状态', power_user.pin_examples);
        console.debug('power_user.strip_examples 状态', power_user.strip_examples);
    ...
    $('#auto_swipe_blacklist').on('input', function () {
        power_user.auto_swipe_blacklist = String($(this).val())
            ...
            .map(str => str.trim())
            .filter(str => str);
        console.log('power_user.auto_swipe_blacklist 状态', power_user.auto_swipe_blacklist);
        saveSettingsDebounced();
    });
    ...
    $('#enable_worker').on('input', function () {
        power_user.worker_optimization = !!$(this).prop('checked');
        switchWorkerOptimization(true);
        saveSettingsDebounced();
    });

    $('#enable_extension_debug_panel').on('input', function () {
        const value = !!$(this).prop('checked');
        power_user.extension_debug_panel = value;
        switchExtensionDebugPanel();
        saveSettingsDebounced();
    });
    ...
    $('#enableZenSliders').on('input', function () {
        ...
        if (power_user.enableLabMode === true && value === true) {
            ...
            toastr.warning('启用 Zen 滑块前请先关闭 Mad Lab 模式');
    ...
    $('#enable_virtualization').on('input', function () {
        const value = !!$(this).prop('checked');
        power_user.enable_virtualization = value;
        switchVirtualization();
        saveSettingsDebounced();
        notifyVirtualizationToggle(value);
    });
    ...
    $('#enableLabMode').on('input', function (event, { fromInit = false } = {}) {
        ...
        if (power_user.enableZenSliders === true && value === true) {
            ...
            toastr.warning('启用 Mad Lab 模式前请先关闭 Zen 滑块');
            $(this).prop('checked', false).trigger('input');
            return;
        }
    ...
    $('#ui_preset_import_file').on('change', async function () {
        ...
        try {
            ...
            console.error('导入 UI 主题失败', error);
            const message = error instanceof Error ? error.message : String(error);
            toastr.error(`错误：${message}`, '导入UI主题失败');
        } finally {
            if (inputElement) {
    ...
    $(document).on('click', '#debug_table [data-debug-function]', function () {
        ...
        if (functionRecord) {
            ...
            console.warn(`未找到调试函数 ${functionId}`);
        }
    });
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        callback: (args, value) => {
            ...
            if (!targetSelector) {
                toastr.error(`目标无效：${args.to}`);
                return;
            }

            if (!args.varname) {
                toastr.error('请填写 CSS 变量名');
                return;
            }
            if (!args.varname.startsWith('--')) {
                toastr.error('CSS 变量名必须以前缀 "--" 开头');
                return;
            }

            const elements = document.querySelectorAll(targetSelector);
            if (elements.length === 0) {
                toastr.error(`未找到 ${args.to ?? 'chat'} 使用选择器 "${targetSelector}" 对应的元素`);
                return;
            }
            ...
            console.info(`已在 "${targetSelector}" 上将 CSS 变量 "${args.varname}" 设置为 "${value}"`);
        },
        namedArgumentList: [
            SlashCommandNamedArgument.fromProps({
                name: 'varname',
