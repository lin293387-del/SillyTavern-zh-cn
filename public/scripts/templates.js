...
function resolveTemplatePath(templateId, fullPath = false) {
...
export async function renderTemplateAsync(templateId, templateData = {}, sanitize = true, localize = true, fullPath = false) {
    ...
    try {
        const pathToTemplate = resolveTemplatePath(templateId, fullPath);
...
export function clearTemplateCache(templateId = null, fullPath = false) {
    if (templateId === null) {
        TEMPLATE_CACHE.clear();
        return true;
    }
    const path = resolveTemplatePath(templateId, fullPath);
    return TEMPLATE_CACHE.delete(path);
}

export function hasTemplateCached(templateId, fullPath = false) {
    const path = resolveTemplatePath(templateId, fullPath);
    return TEMPLATE_CACHE.has(path);
}
