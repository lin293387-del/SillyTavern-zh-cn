...
export async function populateFileAttachment(message, inputId = 'file_form_input') {
    try {
        ...
        // If file is video
            ...
            if (isConvertible(file.type)) {
                try {
                    ...
                    toastr.error(String(error), t`无法转换文件`);
                    console.error('无法转换文件', error);
                }
            }
        ...
        console.error('无法上传文件', error);
        toastr.error(t`文件已损坏或格式不支持`, t`无法上传文件`);
    } finally {
...
export async function uploadFileAttachment(fileName, base64Data) {
    try {
        ...
        toastr.error(String(error), t`无法上传文件`);
        console.error('无法上传文件', error);
    }
}
...
async function validateFile(file) {
    ...
    if (!isImage && file.size > fileSizeLimit) {
        toastr.error(t`文件太大，最大尺寸不能超过 ${humanFileSize(fileSizeLimit)}`);
        return false;
    }

    // If file is binary
    if (isBinary && !isImage && !isConvertible(file.type)) {
        toastr.error(t`不支持二进制文件。请选择一个txt文件或图像`);
        return false;
    }
...
function embedMessageFile(messageId, messageBlock) {
    ...
    if (!message) {
        console.warn('未能找到此 ID 对应的消息', messageId);
        return;
    }
...
export function decodeStyleTags(text, { prefix } = { prefix: '.mes_text ' }) {
    ...
    function extractImportUrl(rule) {
        const importValue = rule?.import || '';
        const urlMatch = importValue.match(/url\(([^)]+)\)/i);
        if (urlMatch && urlMatch[1]) {
            return urlMatch[1].replace(/['"]/g, '').trim();
        }
        const quotedMatch = importValue.match(/(['"])([^'"]+)\1/);
        if (quotedMatch && quotedMatch[2]) {
            return quotedMatch[2].trim();
        }
        return importValue.replace(/['"]/g, '').trim();
    }

    function sanitizeRule(rule, externalImports) {
    ...
    function sanitizeRuleSet(ruleSet, externalImports) {
        if (Array.isArray(ruleSet.selectors) || Array.isArray(ruleSet.declarations)) {
            sanitizeRule(ruleSet, externalImports);
        }

        if (Array.isArray(ruleSet.rules)) {
            const retainedRules = [];

            for (const nestedRule of ruleSet.rules) {
                if (nestedRule.type === 'import') {
                    const url = extractImportUrl(nestedRule);
                    if (url && /^https?:/i.test(url)) {
                        externalImports.push(url);
                        continue;
                    }
                }
                sanitizeRuleSet(nestedRule, externalImports);
                retainedRules.push(nestedRule);
            }

            ruleSet.rules = retainedRules;
        }
    }
    ...
    return text.replaceAll(styleDecodeRegex, (_, style) => {
        try {
            ...
            if (sheet) {
                sanitizeRuleSet(ast.stylesheet, externalImports);
            }
            const sanitizedCss = css.stringify(ast);

            let styleAnchor = '';
            if (sanitizedCss) {
                const styleHash = getStringHash(sanitizedCss);
                registerDynamicStyle(styleHash, sanitizedCss);
                styleAnchor = `<span class="mes_style_anchor" data-style-ref="${styleHash}"></span>`;
            }

            let importAnchor = '';
            if (externalImports.length) {
                const encoded = encodeURIComponent(externalImports.join(','));
                const priority = externalImports.some(url => /highlight|code|prism|monaco/i.test(url))
                    ? 0
                    : EXTERNAL_IMPORT_DEFAULT_PRIORITY;
                importAnchor = `<span class="style_import_anchor" data-style-imports="${encoded}" data-style-priority="${priority}"></span>`;
            }

            return `${styleAnchor}${importAnchor}`;
        } catch (error) {
...
async function openGlobalStylesPreferenceDialog() {
    if (selected_group) {
        toastr.info(t`要更改全局样式，请单独选择一个字符`);
        return;
    }
...
async function deleteMessageImage() {
    ...
    if (isLastImage || value === POPUP_RESULT.CUSTOM1) {
        ...
        mesBlock.find('.mes_img_container').remove();
        mesBlock.find('.mes_text').removeClass('displayNone');
    } else {
...
async function deleteMessageVideo() {
    const confirm = await Popup.show.confirm(t`从消息中删除视频？`, t`此操作无法取消`);
    if (!confirm) {
        return;
    }
...
async function deleteFileFromServer(url, silent = false) {
    try {
        ...
        toastr.error(String(error), t`无法删除文件`);
...
async function moveAttachment(attachment, source, callback) {
    ...
    if (result !== POPUP_RESULT.AFFIRMATIVE) {
        console.debug('已取消移动附件');
        return;
    }

    if (selectedTarget === source) {
        console.debug('已取消移动附件，原位置与目标位置相同');
        return;
    }
...
async function runScraper(scraperId, target, callback) {
    try {
        ...
        if (!Array.isArray(files)) {
            console.warn('没有抓取到任何内容');
            return;
        }

        if (files.length === 0) {
            console.warn('没有抓取到任何内容');
            toastr.info(t`没有文件被抓取.`, t`数据库`);
            return;
        }
        ...
        toastr.success(t`已从 ${scraperId} 抓取 ${files.length} 个文件并保存到 ${target}`, t`数据库`);
        callback();
    }
    catch (error) {
        console.error('抓取失败', error);
        toastr.error(t`查看浏览器控制台以获取详细信息`, t`抓取失败`);
    }
}
    ...
    try {
        ...
        for (const attachment of attachments) {
            if (verifiedUrls[attachment.url] === false) {
                console.log('正在删除孤立的文件', attachment);
                await deleteAttachment(attachment, source, () => { }, false);
            }
        }
    } catch (error) {
        ...
        switch (node.tagName) {
            ...
            case 'IMG': {
                ...
                if (src && isExternalUrl(src)) {
                    console.warn('外部媒体被阻止', src);
                    mediaBlocked = true;
                    node.remove();
                }

                if (data && isExternalUrl(data)) {
                    console.warn('外部媒体被阻止', data);
                    mediaBlocked = true;
                    node.remove();
                }
...
export function initChatUtilities() {
    ...
    chatContainer.on('click', '.mes_hide', async function () {
        const messageBlock = $(this).closest('.mes');
        const messageId = Number(messageBlock.attr('mesid'));
        await hideChatMessageRange(messageId, messageId, false);
    });

    chatContainer.on('click', '.mes_unhide', async function () {
        const messageBlock = $(this).closest('.mes');
        const messageId = Number(messageBlock.attr('mesid'));
        await hideChatMessageRange(messageId, messageId, true);
    });

    chatContainer.on('click', '.mes_file_delete', async function () {
        const messageBlock = $(this).closest('.mes');
        const messageId = Number(messageBlock.attr('mesid'));
        await deleteMessageFile(messageId);
    });

    chatContainer.on('click', '.mes_file_open', async function () {
        const messageBlock = $(this).closest('.mes');
        const messageId = Number(messageBlock.attr('mesid'));
        await viewMessageFile(messageId);
    });

    chatContainer.on('click', '.assistant_note_export', async function () {
        const chatToSave = [
            {
                user_name: name1,
    ...
    chatContainer.on('click', '.assistant_note_import', async function () {
        const importFile = async () => {
            ...
            try {
                ...
                console.error('导入助手聊天时出错', error);
                toastr.error(t`它已经损坏或不是有效的jsonl文件`, t`聊天导入失败`);
            }
        };
    ...
    doc.on('click', '#attachFile', function () {
        $('#file_form_input').trigger('click');
    });

    // Do not change. #manageAttachments is added by extension.
    doc.on('click', '#manageAttachments', function () {
        openAttachmentManager();
    });

    chatContainer.on('click', '.mes_embed', function () {
        const messageBlock = $(this).closest('.mes');
        const messageId = Number(messageBlock.attr('mesid'));
        embedMessageFile(messageId, messageBlock);
    });

    chatContainer.on('click', '.editor_maximize', async function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        if (!bro.length) {
            console.error('无法找到具有该 ID 的编辑器', broId);
            return;
        }
    ...
    chatContainer.on('click', '.mes .mes_text', function () {
    ...
    chatContainer.on('click', '.open_media_overrides', openExternalMediaOverridesDialog);
    doc.on('input', '#forbid_media_override_allowed', function () {
        const entityId = getCurrentEntityId();
        if (!entityId) return;
    ...
    doc.on('input', '#forbid_media_override_forbidden', function () {
        const entityId = getCurrentEntityId();
        if (!entityId) return;
    ...
    doc.on('input', '#forbid_media_override_global', function () {
        const entityId = getCurrentEntityId();
        if (!entityId) return;
        power_user.external_media_allowed_overrides = power_user.external_media_allowed_overrides.filter((v) => v !== entityId);
        power_user.external_media_forbidden_overrides = power_user.external_media_forbidden_overrides.filter((v) => v !== entityId);
    ...
    chatContainer.on('click', '.mes_img', expandMessageImage);
    chatContainer.on('click', '.mes_img_enlarge', expandAndZoomMessageImage);
    chatContainer.on('click', '.mes_img_delete', deleteMessageImage);
    chatContainer.on('click', '.mes_video_delete', deleteMessageVideo);
