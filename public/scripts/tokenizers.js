...
import { characters, main_api, nai_settings, online_status, this_chid, token } from '../script.js';
...
function buildTokenizerWorkerPayload(type, text) {
    if (type === tokenizers.API_CURRENT) {
        return buildTokenizerWorkerPayload(currentRemoteTokenizerAPI(), text);
    }
    ...
    if (type === tokenizers.API_KOBOLD) {
        return {
            mode: 'remote',
            tokenizerType: type,
            text,
            endpoint: TOKENIZER_URLS[tokenizers.API_KOBOLD]?.count,
            body: {
                text,
    ...
    if (type === tokenizers.API_TEXTGENERATIONWEBUI) {
        return {
            mode: 'remote',
            tokenizerType: type,
            text,
            endpoint: TOKENIZER_URLS[tokenizers.API_TEXTGENERATIONWEBUI]?.count,
            body: getTextgenAPITokenizationParams(text),
        };
    }
...
function buildOpenAIWorkerPayload(messages, model, full, cacheObject) {
    const endpoint = `/api/tokenizers/openai/count?model=${model}`;
    const normalizedMessages = Array.isArray(messages) ? messages : [messages];

    const payloadMessages = normalizedMessages.map((message) => {
        ...
        return {
            message,
            cacheKey,
            cachedCount: typeof cachedCount === 'number' ? cachedCount : null,
        };
    });
...
async function dispatchTokenizerTask(payload) {
    if (!payload) {
        return { count: 0 };
    }

    if (payload.mode === 'guesstimate') {
        return { count: guesstimate(payload.text || '') };
    }

    return tokenizerWorkerFallback(payload);
}
...
export async function getTokenCountAsync(str, padding = undefined) {
    ...
    if (main_api === 'openai') {
        if (padding === power_user.token_padding) {
            ...
            const message = { role: 'system', content: str };
            const model = getTokenizerModel();
            const workerPayload = buildOpenAIWorkerPayload(message, model, true, cacheObject);
            const response = await dispatchTokenizerTask(workerPayload);
            const total = Number(response?.count ?? 0);

            if (Array.isArray(response?.cacheUpdates)) {
                for (const entry of response.cacheUpdates) {
                    if (entry && typeof entry.cacheKey === 'string' && typeof entry.count === 'number') {
                        cacheObject[entry.cacheKey] = entry.count;
                    }
                }
            }
    ...
    const payload = buildTokenizerWorkerPayload(tokenizerType, str);
    const response = await dispatchTokenizerTask(payload);
    const baseCount = Number(response?.count ?? 0);
    ...
    const result = baseCount + padding;
...
function countTokensFromServerAsync(endpoint, text) {
    return new Promise((resolve) => {
        countTokensFromServer(endpoint, text, resolve);
    });
}

function countTokensFromKoboldAsync(text) {
    return new Promise((resolve) => {
        countTokensFromKoboldAPI(text, resolve);
    });
}

function countTokensFromTextgenAsync(text) {
    return new Promise((resolve) => {
        countTokensFromTextgenAPI(text, resolve);
    });
}

async function tokenizerWorkerFallback(payload) {
    if (!payload) {
        return { count: 0 };
    }

    const { mode, tokenizerType, text } = payload;
    ...
    try {
        switch (mode) {
            ...
            case 'openai': {
                const messages = payload.messages?.map(item => item?.message) || [];
                const total = await countTokensOpenAIAsync(messages, payload.full);
                return { count: Number(total) || 0 };
            }
            case 'remote': {
                if (tokenizerType === tokenizers.API_KOBOLD) {
                    const count = await countTokensFromKoboldAsync(text);
                    return { count: Number(count) || 0 };
                }

                if (tokenizerType === tokenizers.API_TEXTGENERATIONWEBUI) {
                    const count = await countTokensFromTextgenAsync(text);
                    return { count: Number(count) || 0 };
                }

                if (payload.endpoint) {
                    const count = await countTokensFromServerAsync(payload.endpoint, text);
                    return { count: Number(count) || 0 };
                }
        ...
        console.error('Tokenizer fallback execution failed', error);
        return { count: 0 };
    }
}
