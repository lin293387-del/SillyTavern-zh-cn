...
import { registerWorkerTask, WorkerFallbackStrategy } from './workers/worker-protocol.js';

const MACRO_WORKER_TASK = 'macro-env-replace';

registerWorkerTask(MACRO_WORKER_TASK, {
    script: 'macro-worker.js',
    fallback: macroEnvFallback,
});
...
function applyMacroList(content, macros, postProcessFn) {
    ...
    for (const macro of macros) {
        ...
        if (!macro?.regex || typeof macro.replace !== 'function') {
            continue;
        }
        ...
        try {
            result = result.replace(macro.regex, (...args) => postProcessFn(macro.replace(...args)));
        } catch (error) {
...
function buildMacroStages(content, env, postProcessFn) {
    const postProcess = typeof postProcessFn === 'function' ? postProcessFn : (x => x);
...
export function evaluateMacros(content, env, postProcessFn) {
    ...
    const stages = buildMacroStages(content, env, postProcessFn);
    let result = applyMacroList(stages.initialContent, stages.preEnvMacros, stages.postProcess);
    result = applyMacroList(result, stages.envMacros, stages.postProcess);
    result = applyMacroList(result, stages.postEnvMacros, stages.postProcess);

    return result;
}

async function applyEnvMacrosWithWorker(content, envMacros, postProcessFn) {
    if (!envMacros.length) {
        return content;
    }

    if (!workerManager.isEnabled() || !workerManager.isSupported()) {
        return applyMacroList(content, envMacros, postProcessFn);
    }

    const replacements = [];

    for (const macro of envMacros) {
        if (!macro?.regex || typeof macro.replace !== 'function') {
            continue;
        }
        ...
        try {
            replacementValue = postProcessFn(macro.replace());
        } catch (error) {
            console.warn('[Macro Worker] 无法预计算替换值', macro.regex, error);
            replacementValue = '';
        }

        replacements.push({
            pattern: macro.regex.source,
            flags: macro.regex.flags || 'g',
    ...
    try {
        const response = await workerManager.dispatchTask(MACRO_WORKER_TASK, {
            content,
            replacements,
        }, {
        ...
        console.warn('[Macro Worker] 调用失败，改用主线程处理', error);
        return applyMacroList(content, envMacros, postProcessFn);
    }
}

export async function evaluateMacrosAsync(content, env, postProcessFn) {
    if (!content) {
        return '';
    }
    ...
    const stages = buildMacroStages(content, env, postProcessFn);
    let result = applyMacroList(stages.initialContent, stages.preEnvMacros, stages.postProcess);
    result = await applyEnvMacrosWithWorker(result, stages.envMacros, stages.postProcess);
    result = applyMacroList(result, stages.postEnvMacros, stages.postProcess);

    return result;
}

function macroEnvFallback(payload) {
    if (!payload || !Array.isArray(payload.replacements)) {
        return { content: String(payload?.content ?? '') };
    }

    let result = String(payload.content ?? '');
    ...
    for (const item of payload.replacements) {
        ...
        try {
            ...
            console.warn('[Macro Worker] 回退处理失败', item.pattern, error);
        }
    }
