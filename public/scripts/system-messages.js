import { addOneMessage, chat, displayVersion, prepareMessageRender, setSendButtonState, system_avatar, systemUserName } from '../script.js';
 ...
 * @param {object} [extra] Additional data to be added to the message
 */
export async function sendSystemMessage(type, text, extra = {}) {
    const newMessage = getSystemMessageByType(type, text, extra);
    chat.push(newMessage);
    const messageId = chat.length - 1;
    const formattedText = newMessage?.extra?.display_text ?? newMessage?.mes ?? '';
    const sanitizerOverrides = newMessage?.uses_system_ui ? { MESSAGE_ALLOW_SYSTEM_UI: true } : {};
    const isReasoning = newMessage?.extra?.type === 'reasoning';
    await prepareMessageRender(formattedText, newMessage?.name, newMessage?.is_system, newMessage?.is_user, messageId, sanitizerOverrides, isReasoning);
