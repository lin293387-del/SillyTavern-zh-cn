...
import { download, debounce, initScrollHeight, resetScrollHeight, getCharaFilename, getSortableDelay, escapeRegex, PAGINATION_TEMPLATE, navigation_option, waitUntilCondition, isTrueBoolean, setValueByPath, flashHighlight, select2ModifyOptions, getSelect2OptionId, dynamicSelect2DataViaAjax, highlightRegex, select2ChoiceClickSubscribe, isFalseBoolean, getSanitizedFilename, checkOverwriteExistingData, getStringHash, parseStringArray, cancelDebounce, findChar, onlyUnique, equalsIgnoreCaseAndAccents } from './utils.js';
...
let currentWorldEntriesRender = null;
...
export function refreshWorldInfoEditor(navigation = navigation_option.none, flashOnNav = true) {
    return updateEditor(navigation, flashOnNav);
}
...
async function displayWorldEntries(name, data, navigation = navigation_option.none, flashOnNav = true) {
    ...
    $('#world_popup_delete').off('click').on('click', async () => {
        const confirmation = await Popup.show.confirm(`删除世界书: "${name}"?`, '此操作不可逆！');
        if (!confirmation) {
            return;
        }
    ...
    $('#world_info_pagination').pagination({
        ...
        callback: async function (/** @type {object[]} */ page) {
            try {
                if (typeof currentWorldEntriesRender?.cancel === 'function') {
                    currentWorldEntriesRender.cancel();
                } else if (currentWorldEntriesRender) {
                    currentWorldEntriesRender.cancelled = true;
                }
                ...
                currentWorldEntriesRender = renderController;
                ...
                const processEntries = async () => {
                    ...
                    for (const entry of page) {
                        if (renderController.cancelled) {
                            return;
                        }
                        try {
                            const block = await getWorldEntry(name, data, entry);
                            if (!block || renderController.cancelled) {
                                continue;
                            }
                            if (!isCustomOrder) {
                        ...
                        processed++;
                        if (processed < page.length && processed % BATCH_SIZE === 0) {
                    ...
                    worldEntriesList.append(fragment);
                    if (currentWorldEntriesRender === renderController) {
                        currentWorldEntriesRender = null;
                    }
                };

                renderController.animationHandle = requestAnimationFrame(() => {
                    renderController.animationHandle = null;
                    processEntries().catch((error) => {
                        console.error('Error while processing WI entry batch:', error);
                    });
                });
            } catch (error) {
...
function setupWorldPopupCleanupObserver() {
    const popup = document.getElementById('world_popup');
    if (!popup) {
        return;
    }

    let lastVisible = $(popup).is(':visible');

    const performCleanupIfHidden = () => {
        ...
        if (lastVisible && !isVisible) {
            ...
            if (entriesList.length) {
                clearEntryList(entriesList);
                entriesList.hide();
            }
            const paginationWidget = $('#world_info_pagination');
            if (typeof paginationWidget.pagination === 'function') {
                paginationWidget.pagination('destroy');
            }
        }
    ...
    $(popup).on('popupHidden', performCleanupIfHidden);
}

$(setupWorldPopupCleanupObserver);
...
export async function renameWorldInfo(oldName, data, options = {}) {
    const { newName: providedNewName, overwrite = false, interactive = true } = options;
    const sourceName = typeof oldName === 'string' ? oldName.trim() : '';

    if (!sourceName) {
        console.error('[WI] renameWorldInfo requires a valid source name');
        return false;
    }

    let worldData = data;
    if (!worldData || typeof worldData !== 'object') {
        worldData = await loadWorldInfo(sourceName);
        if (!worldData) {
            console.error(`[WI] Could not load lorebook "${sourceName}" for rename.`);
            return false;
        }
    }

    let targetName = providedNewName;
    if (!targetName && interactive) {
        targetName = await Popup.show.input('Rename World Info', 'Enter a new name:', sourceName);
    }

    if (!targetName) {
        console.debug('[WI] World info rename cancelled or no target name specified.');
        return false;
    }

    targetName = String(targetName).trim();
    if (!targetName) {
        toastr.warning(t`Name not accepted, it cannot be empty.`, t`Rename World Info`);
        return false;
    }

    const sanitizedTarget = await getSanitizedFilename(targetName);

    if (equalsIgnoreCaseAndAccents(sourceName, sanitizedTarget)) {
        toastr.warning(t`Name not accepted, as it is the same as before (ignoring case and accents).`, t`Rename World Info`);
        return false;
    }

    if (!overwrite && world_names.includes(sanitizedTarget) && !equalsIgnoreCaseAndAccents(sourceName, sanitizedTarget)) {
        if (interactive) {
            toastr.warning(t`A lorebook with that name already exists. Please choose a different name.`, t`Rename World Info`);
            return false;
        }
        throw new Error(`Lorebook "${sanitizedTarget}" already exists.`);
    }
    ...
    const entryPreviouslySelected = selected_world_info.findIndex((e) => e === sourceName);

    worldData.name = sanitizedTarget;
    await saveWorldInfo(sanitizedTarget, worldData, true);
    await deleteWorldInfo(sourceName);

    const existingCharLores = world_info.charLore?.filter((e) => e.extraBooks.includes(sourceName));
    if (existingCharLores && existingCharLores.length > 0) {
        existingCharLores.forEach((charLore) => {
            const tempCharLore = charLore.extraBooks.filter((e) => e !== sourceName);
            tempCharLore.push(sanitizedTarget);
            charLore.extraBooks = tempCharLore;
        });
        saveSettingsDebounced();
    }
    ...
    if (entryPreviouslySelected !== -1) {
        const wiElement = getWIElement(sanitizedTarget);
        wiElement.prop('selected', true);
        $('#world_info').trigger('change');
    }

    const selectedIndex = world_names.indexOf(sanitizedTarget);
    if (selectedIndex !== -1) {
        $('#world_editor_select').val(selectedIndex).trigger('change');
    }
    ...
    if (characters[chid]?.data?.character_book) {
        ...
        if (!accountStorage.getItem(checkKey) && (!worldName || !world_names.includes(worldName))) {
            ...
            if (power_user.world_import_dialog) {
                const html = `<h3>此角色有一个内嵌的世界书。你想现在导入吗？</h3>
                <div class="m-b-1">如果你想稍后导入，请点击
                    <span class="inline-flex alignitemscenter gap5px">
                        <span class="menu_button fa-solid fa-globe interactable world_set" title="角色世界书 
单击加载
Shift+单击打开“链接到世界信息”弹出窗口" data-i18n="[title]world_button_title" tabindex="0" role="button"></span>
                    </span>
                  </div>`;
...
export async function importEmbeddedWorldInfo(skipPopup = false) {
    ...
    if (!skipPopup) {
        const confirmation = await Popup.show.confirm(`你确定要导入世界书 '${bookName}' 吗？`, world_names.includes(bookName) ? '这将覆盖同名的世界书。' : '');
        if (!confirmation) {
            return;
        }
    }
    ...
    toastr.success(`世界书 '${bookName}' 已成功导入并绑定角色。`, '世界书导入');
...
export function initWorldInfo() {
    ...
    const emitWorldInfoSettingsUpdated = (() => {
        let scheduled = false;
        return () => {
            if (scheduled) {
                return;
    ...
    const queueWorldInfoSettingsSave = () => {
        saveSettingsDebounced();
        emitWorldInfoSettingsUpdated();
    };

    $('#world_info_depth').on('input', function () {
        world_info_depth = Number($(this).val());
        $('#world_info_depth_counter').val($(this).val());
        queueWorldInfoSettingsSave();
    });
