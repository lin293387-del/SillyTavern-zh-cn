import { chat_metadata, getCurrentChatId, variableService } from '../script.js';
import { extension_settings } from './extensions.js';
import { VARIABLE_SCOPE } from './variable-service.js';
...
export function getLocalVariable(name, args = {}) {
    ...
    try {
        localVariable = variableService.get(VARIABLE_SCOPE.CHAT, key, { clone: true });
    } catch (error) {
        console.debug('[variables] 读取本地变量时回退至直接访问', error);
        localVariable = chat_metadata.variables[key];
    }

    if (args.index !== undefined) {
        try {
            const serialized = typeof localVariable === 'string'
                ? localVariable
                : JSON.stringify(localVariable ?? null);
            let parsed = JSON.parse(serialized ?? 'null');
            const numIndex = Number(args.index);
            if (Number.isNaN(numIndex)) {
                parsed = parsed?.[args.index];
            } else {
                parsed = parsed?.[Number(args.index)];
            }
            localVariable = typeof parsed === 'object' ? JSON.stringify(parsed) : parsed;
        } catch {
            // ignore parse errors, fall back to raw value
        }
    }
...
export function getGlobalVariable(name, args = {}) {
    ...
    try {
        globalVariable = variableService.get(VARIABLE_SCOPE.GLOBAL, key, { clone: true });
    } catch (error) {
        console.debug('[variables] 读取全局变量时回退至直接访问', error);
        globalVariable = extension_settings.variables.global[key];
    }

    if (args.index !== undefined) {
        try {
            const serialized = typeof globalVariable === 'string'
                ? globalVariable
                : JSON.stringify(globalVariable ?? null);
            let parsed = JSON.parse(serialized ?? 'null');
            const numIndex = Number(args.index);
            if (Number.isNaN(numIndex)) {
                parsed = parsed?.[args.index];
            ...
            globalVariable = typeof parsed === 'object' ? JSON.stringify(parsed) : parsed;
        } catch {
            // ignore parse errors, fall back to raw value
        }
    }
...
function existsLocalVariable(name) {
    try {
        return variableService.get(VARIABLE_SCOPE.CHAT, name, { clone: false }) !== undefined;
    } catch {
...
function existsGlobalVariable(name) {
    try {
        return variableService.get(VARIABLE_SCOPE.GLOBAL, name, { clone: false }) !== undefined;
    } catch {
...
function deleteLocalVariable(name) {
    ...
    variableService.remove(VARIABLE_SCOPE.CHAT, name);
    return '';
}
...
function deleteGlobalVariable(name) {
    ...
    variableService.remove(VARIABLE_SCOPE.GLOBAL, name);
    return '';
}
