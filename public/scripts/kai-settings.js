...
defineSaveScheduler('kobold-settings', {
    panelId: 'kobold-settings',
    triggers: ['input', 'change'],
    strategy: 'raf',
});
const queueKoboldSave = createSaveScheduler('kobold-settings', () => createPanelSaveScheduler(() => saveSettingsDebounced()));
...
export async function getStatusKobold() {
    ...
    if (!isBackendApiEnabled('kobold')) {
        if (backendApiLoggingEnabled) {
            console.info('[API toggle] Kobold API disabled，跳过状态检查。');
        }
        setOnlineStatus('no_connection');
        return resultCheckStatus();
    }
    ...
    try {
        ...
        if (!response.ok) {
            if (backendApiLoggingEnabled) {
                console.info(`[API toggle] Kobold 状态请求被阻止：${response.status}`);
            }
            setOnlineStatus('no_connection');
            return resultCheckStatus();
        }
