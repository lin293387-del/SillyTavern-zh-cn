...
import { debounce, getStringHash, isValidUrl, throttle } from './utils.js';
...
export function dragElement($elmnt) {
    ...
    function applyPendingStyles() {
        ...
        for (const [prop, value] of Object.entries(pendingStyles)) {
            elementNode.style[prop] = value;
        }
        pendingStyles = {};
    }

    /**
     * Queue a single style write for the element.
     * @param {string} prop
     * @param {string} value
     */
    function queueStyle(prop, value) {
        if (pendingStyles[prop] === value) {
            return;
        }
        pendingStyles[prop] = value;
        if (pendingStyleFrame === null) {
            pendingStyleFrame = requestAnimationFrame(applyPendingStyles);
        }
    }

    /**
     * Queue multiple style writes for the element.
     * @param {Record<string, string>} updates
     */
    function queueStyles(updates) {
        let hasUpdate = false;
        ...
        if (hasUpdate && pendingStyleFrame === null) {
            pendingStyleFrame = requestAnimationFrame(applyPendingStyles);
        }
    }

    function flushQueuedStyles() {
        if (pendingStyleFrame !== null) {
    ...
    function clampToViewport() {
        if (top <= 0) {
            ...
            const clampedTop = Math.max(0, winHeight - maxY + top - 1);
            queueStyle('top', `${clampedTop}px`);
        }
        if (left <= 0) {
            queueStyle('left', '0px');
        } else if (maxX >= winWidth) {
            const clampedLeft = Math.max(0, winWidth - maxX + left - 1);
            queueStyle('left', `${clampedLeft}px`);
        }
    }
        ...
        if (actionType === 'resize') {
            ...
            if ($elmnt.attr('id').startsWith('zoomFor_')) {
                ...
                if (containerAspectRatio !== imageAspectRatio) {
                    const currentWidth = $elmnt.width();
                    const adjustedHeight = currentWidth * imageAspectRatio;
                    queueStyles({
                ...
                if (top + $elmnt.height() >= winHeight) {
                    const clampedHeight = Math.max(0, winHeight - top - 1);
                    queueStyles({
                        height: `${clampedHeight}px`,
                        width: `${clampedHeight / imageAspectRatio}px`,
                    });
                }
                if (left + $elmnt.width() >= winWidth) {
                    const clampedWidth = Math.max(0, winWidth - left - 1);
                    queueStyles({
                        width: `${clampedWidth}px`,
                        height: `${clampedWidth * imageAspectRatio}px`,
                    });
                }
            } else {
                if (top + $elmnt.height() >= winHeight) {
                    const clampedHeight = Math.max(0, winHeight - top - 1);
                    queueStyle('height', `${clampedHeight}px`);
                }
                if (left + $elmnt.width() >= winWidth) {
                    const clampedWidth = Math.max(0, winWidth - left - 1);
                    queueStyle('width', `${clampedWidth}px`);
                }
            }
    ...
    const elementDragCore = (e) => {
        ...
        const newLeft = $elmnt.offset().left - pos1;
        const newTop = $elmnt.offset().top - pos2;
        if (height === undefined || Number.isNaN(height)) {
            height = $elmnt.outerHeight();
        }
        if (width === undefined || Number.isNaN(width)) {
            width = $elmnt.outerWidth();
        }
        queueStyles({
            left: `${newLeft}px`,
            top: `${newTop}px`,
    ...
    const elementDrag = throttle((event) => {
        elementDragCore(event);
    }, 16);
    ...
    function closeDragElement() {
        ...
        if (typeof elementDrag.cancel === 'function') {
            elementDrag.cancel();
        }
...
export function initRossMods() {
    ...
    if (power_user.auto_load_chat) {
        const triggerAutoLoad = () => {
            ...
            if (!active_character && !active_group) {
                return;
            }

            autoLoadPromise = RA_autoloadchat();
        };

        autoLoadPromise = RA_autoloadchat();

        eventSource.once(event_types.APP_READY, () => {
            if (autoLoadPromise && typeof autoLoadPromise.then === 'function') {
                autoLoadPromise.finally(triggerAutoLoad);
            } else {
                triggerAutoLoad();
            }
        });
