...
import { delegateLeftNav, createPanelSaveScheduler } from './util/panel-delegation.js';
import { defineSaveScheduler, createSaveScheduler } from './util/save-scheduler.js';

defineSaveScheduler('textgen-settings', {
    panelId: 'textgen-settings',
    triggers: ['input', 'change'],
    strategy: 'raf',
});
const queueTextgenSave = createSaveScheduler('textgen-settings', () => createPanelSaveScheduler(() => saveSettingsDebounced()));
...
export function initTextGenSettings() {
    ...
    for (const settingId of setting_names) {
        const selector = `#${settingId}_textgenerationwebui`;
        $(selector).attr('x-setting-id', settingId);
        delegateLeftNav('input', selector, function () {
