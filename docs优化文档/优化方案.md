...
- `public/script.js:2460-2730` 引入右侧面板 VirtualList 管理（`initRightPanelVirtualList`、`setRightPanelVirtualItems`、`ensureCharacterCardVisible/ensureGroupCardVisible`），角色与群组卡片通过统一注册表与等待队列复用 DOM，滚动仅渲染视窗窗口，默认 overscan 为 5/8，兼容列表视图与网格视图。
- 同段落保留 `renderLegacyList` 兜底：当虚拟化关闭、卡片总数较少或开启批量选择（`#rm_print_characters_block.bulk_select`）时自动退回原始 append 流程，保证批量操作、旧浏览器及手动关闭场景行为一致。
- `public/script.js:13970-14060` 的 `select_rm_info` 改为调用新 helper，创建/导入角色或群组后会等待虚拟化窗口准备并滚动到目标卡片，高亮逻辑不再依赖静态 DOM 查询。
- `public/scripts/RossAscends-mods.js:300-320` 自动加载最近角色时使用 `ensureCharacterCardVisible`，扩展级标签同步不再因为虚拟化移除节点而失效。
- `public/scripts/bulk-edit.js:9-140` 进入/退出批量编辑时先切换 `bulk_select` 标记后调用 `printCharacters(false)` 重新渲染，批量模式下自动禁用虚拟化并重新生成复选框，离开后恢复虚拟化及事件委托。
- `public/script.js:2560-2610` 的 `ensureBulkCheckboxForElement` 在虚拟化仍开启但容器处于批量模式时补齐复选框，保持旧代码兼容。
- 新增导出 `getCharacterCardDom/hasCharacterCardInVirtualList/ensureGroupCardVisible` 等工具，为扩展或后续特性提供统一的虚拟化感知入口。
- `public/script.js:2700-2950` 新增左侧抽屉 VirtualList 管理器（`ensureLeftPanelVirtualizer`、`initLeftPanelVirtualization`、`destroyLeftPanelVirtualization` 等），同时引入隐藏缓存区 `data-left-panel-virtual-cache` 与包装容器 `left-panel-virtual-wrapper`，实现 `#ai_response_configuration`、`#AdvancedFormatting` 子节点的窗口化渲染；停泊元素仍保留在 DOM 树内，`document.getElementById`、`delegatePanelEvent` 等既有依赖保持可用。
